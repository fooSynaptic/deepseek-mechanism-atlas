#!/usr/bin/env python3
"""Launch one L3 IsoFLOP job on 20 GPUs via c10d rendezvous.

nproc_per_node can differ across nodes. Do not use the 8-GPU packer for these jobs.
Set DS_RDZV_NODES and L3_MASTER_ADDR (defaults to the first node's IP).
"""
...
MASTER = master_addr(NODES, "L3_MASTER_ADDR")
...
def master_port(job_id: str, *, smoke: bool) -> int:
    # Smoke/full must never share a port (crc32(t1_smoke20) collided with crc32(t1)).
    key = f"{job_id}:{'s20' if smoke else 'full'}:v2"
    return 29600 + zlib.crc32(key.encode()) % 80
...
def wait_job_gone(job_id: str, timeout_s: int = 180) -> None:
    """Wait until no train.py remains for job_id on any L3 node (post-smoke / pre-next)."""
    t0 = time.time()
    while time.time() - t0 < timeout_s:
        if not already_running(job_id):
            print(f"[l3-20] job gone {job_id}", flush=True)
            return
        time.sleep(3)
    raise SystemExit(f"timeout waiting for {job_id} processes to exit")


def wait_l3_gpus_idle(timeout_s: int = 300) -> None:
    """Wait until L3 CUDA_VISIBLE_DEVICES cards report low used memory."""
    checks = tuple((host, cvd) for host, _, cvd, _ in NODES)
    t0 = time.time()
    while time.time() - t0 < timeout_s:
        busy = []
        for host, idxs in checks:
            r = ssh(
                host,
                f"nvidia-smi --id={idxs} --query-gpu=index,memory.used --format=csv,noheader,nounits",
                check=False,
            )
            for line in r.stdout.strip().splitlines():
                ...
                try:
                    ...
                    continue
                # Allow a little residual driver / zombie noise; real training is tens of GB.
                if used > 1500:
                    busy.append(f"{host}:gpu{parts[0]}={used:.0f}MiB")
        if not busy:
            print("[l3-20] L3 GPUs idle", flush=True)
            return
        print(f"[l3-20] waiting GPU drain: {', '.join(busy[:4])}", flush=True)
        time.sleep(5)
    raise SystemExit("timeout waiting for L3 GPUs to drain")
...
def launch(job: dict, *, smoke: bool) -> None:
    ...
    rdzv = f"{MASTER}:{port}"
    rdzv_id = f"{jid}_{'s20' if smoke else 'full'}_{int(time.time())}"
    outdir = RESULTS / (f"{jid}.smoke20" if smoke else jid)
    extra = "--no-resume --smoke-steps 8 --eval-batches 1 --log-every 1" if smoke else "--no-resume"
    ...
    print(f"[l3-20] {jid} smoke={smoke} rdzv={rdzv} out={outdir}", flush=True)
    # Truncate per-host logs so wait_smoke does not match a prior Traceback.
    for host, _, _, _ in NODES:
        log = LOGS / f"{jid}.{host}.20gpu{'.smoke' if smoke else ''}.log"
...
"""
        ...
        # Let the first rendezvous host register first so it owns node_rank 0.
        if host == NODES[0][0]:
            time.sleep(5)
...
def wait_smoke(job_id: str, timeout_s: int = 600) -> None:
    log = LOGS / f"{job_id}.{NODES[0][0]}.20gpu.smoke.log"
    t0 = time.time()
    while time.time() - t0 < timeout_s:
        r = ssh(
            NODES[0][0],
        ...
        ok = ssh(
            NODES[0][0],
            f"test -f {RESULTS}/{job_id}.smoke20/result.json && echo Y || echo N",
            check=False,
        ).stdout.strip()
        if ok == "Y":
            ssh(
                NODES[0][0],
                f"mkdir -p {RESULTS}/{job_id}; echo SMOKE20_OK > {RESULTS}/{job_id}/smoke20_ok; echo SMOKE20_OK > {RESULTS}/{job_id}/smoke_ok",
            )
            print(f"[l3-20] smoke ok {job_id}", flush=True)
            wait_job_gone(job_id, timeout_s=180)
            wait_l3_gpus_idle(timeout_s=300)
            return
...
def wait_result(job_id: str) -> None:
    ...
    print(f"[l3-20] waiting {p}", flush=True)
    dead_ticks = 0
    while True:
        r = ssh(NODES[0][0], f"test -f {p} && echo Y || echo N", check=False)
        if r.stdout.strip() == "Y":
            print(f"[l3-20] done {job_id}", flush=True)
            wait_job_gone(job_id, timeout_s=180)
            wait_l3_gpus_idle(timeout_s=300)
            return
        if already_running(job_id):
            ...
            dead_ticks += 1
            # 3 * 30s with no processes and no result.json → hard fail so watcher does not hang forever.
            if dead_ticks >= 3:
                raise SystemExit(
...
def main() -> None:
    ...
    ap.add_argument("--grid", default=str(ROOT / "configs" / "grid_wave_l3.json"))
    ...
    if already_running(args.job_id):
        print(f"[l3-20] already running {args.job_id}", flush=True)
        if args.smoke:
            # Mid-pipe restart: full is already up — do not re-smoke.
            print(f"[l3-20] skip smoke; job already live", flush=True)
            return
        if args.wait:
            wait_result(args.job_id)
        return
    # Always drain before a fresh launch (smoke or full) so t1→t0 handoff cannot race.
    wait_l3_gpus_idle(timeout_s=300)
