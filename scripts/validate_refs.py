...
# Absolute paths and hrefs that resolve outside the repo root.
SKIP = {"embeddings", "key", "query", "节点文档", "链接"}
REPO_ROOT = REPO.resolve()


def _escapes_repo(md: Path, href: str) -> bool:
    """True if href is absolute or resolves outside REPO."""
    norm = href.replace("\\", "/").split("#", 1)[0]
    if not norm:
        return False
    if norm.startswith("/"):
        return True
