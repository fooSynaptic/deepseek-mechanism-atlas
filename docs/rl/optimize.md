...
R1 的 [RLVR + GRPO](../versions/rlvr.md) 针对的是 **同一 prompt、$G$ 条 rollout、终局 verifier 打分**——在短 CoT 上成立。长程任务把 **轨迹长度、上下文压缩、延迟归因** 拉进同一个组内比较，前提就被动摇了。
...
| 资源 | 说明 |
|------|------|
| [RLVR 详解](../versions/rlvr.md) | 可验证奖励 + GRPO 机制 |
| [GRPO vs PPO 对照图](../figures/rl/grpo-vs-ppo.svg) | PPO（critic）vs GRPO（组内 baseline）对照图 |
