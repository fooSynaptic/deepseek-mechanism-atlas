...
> [← 中文导读](../README.md) · [← 仓库首页（EN）](../../README.md) · [← 演进总览 §3.7](../reports/deepseek-version-lineage-20260625.md#37-deepseek-v4) · [← 算法线导读](../reports/deepseek-algorithm-line.md) · [← V4 梗概](./v4.md) · [HC 基础](./hyper-connections.md) · [Raschka §8 mHC](../reports/raschka-technical-deepseek-v3-v32-highlights.md#结论表-7--8)  
...
**mHC** 在 **[Hyper-Connections（HC）](./hyper-connections.md)** 的多路残差流之上，用 **Sinkhorn–Knopp** 把残差混合矩阵投影到 **双随机流形**（[Birkhoff 多面体](./qa/mhc-birkhoff-polytope.md)），使层间信号混合等价于 **凸组合 / 加权平均**，从而 **恢复恒等映射的稳定性**；**DeepSeek-V4** 将其作为残差路径的标准组件。

> **答疑**：[HC（Hyper-Connections）基础](./hyper-connections.md) — $n$ 路并行残差流 + pre/post/comb 混合；裸 HC 破坏恒等映射，mHC 用双随机约束修复
...
| **Hyper-Connections (HC)** | 扩宽残差流为 $n$ 条并行流，可学习混合 | 容量↑，但 **破坏恒等映射** → 训练易爆（[HC 专文](./hyper-connections.md)） |
| **mHC** | HC + **流形约束**（双随机混合矩阵） | 保留 HC 表达力，**抑制信号放大** |

[Raschka 扩展阅读 §8 · 表 8-1](../reports/raschka-technical-deepseek-v3-v32.md#表-8-1-transformer-模块演进) 归纳的 Transformer 模块演进链（节选）（[要点速读](../reports/raschka-technical-deepseek-v3-v32-highlights.md#结论表-7--8)）：

| 模块 | 演进 |
|------|------|
| Attention | GQA → sliding window → **MLA** → **DSA** → **[CSA/HCA](./csa-hca-mixed-attention.md)**（见 [算法线导读](../reports/deepseek-algorithm-line.md)） |
| FFN | GeLU → SwiGLU → **MoE** |
| **残差** | 恒等残差 → **HC** → **mHC** |

> **扩展阅读**：[Raschka 全文 §8 — Transformer 模块演进表](../reports/raschka-technical-deepseek-v3-v32.md#表-8-1-transformer-模块演进) · [要点速读 §7–8](../reports/raschka-technical-deepseek-v3-v32-highlights.md#结论表-7--8)
...
| 方向 | 文档 |
|------|------|
| **本节点（④ mHC）** | [算法线导读 §1](../reports/deepseek-algorithm-line.md#1-演进链attention--残差) · [HC 基础](./hyper-connections.md) · [§3 双随机流形](#3-mhc-核心双随机流形约束) |
| **同代 Attention** | [csa-hca-mixed-attention.md](./csa-hca-mixed-attention.md) · [v4.md](./v4.md) |
| **Attention 上游** | [mla-latent-attention.md](./mla-latent-attention.md) · [dsa-sparse-attention.md](./dsa-sparse-attention.md) |

---

## 2. Hyper-Connections（HC）基础

<a id="hc-basics"></a>

标准残差 → **$n$ 路并行残差流** + **pre / post / comb** 混合 → 子层 Attention/FFN；裸 HC **破坏恒等映射**，深层可出现 **~3000×** 信号放大。

> **专文**：[Hyper-Connections（HC）— 多路残差流 · 混合矩阵 · 不稳定原因](./hyper-connections.md)
...
mHC 把 [HC](./hyper-connections.md) 的可学习混合矩阵 **投影** 到 **双随机流形**（Birkhoff 多面体 $\mathcal{B}_n$）：非负、行和列均为 1 → 对 $n$ 条残差流做 **凸组合**，抑制无约束 HC 的 **~3000×** 级信号放大，恢复深层 **近似恒等映射** 的稳定性。
...
[直接打开 SVG](../figures/mhc/mhc-doubly-stochastic-matrix.svg)
...
[HC](./hyper-connections.md) 把每层残差扩成 $n$ 条并行流，用可学习矩阵 $H_l^{\mathrm{res}}$ 混合。若 **不约束** $H_l^{\mathrm{res}}$：

- 复合映射 $\prod_{l=1}^{L} H_l^{\mathrm{res}}$ 的谱范数可 **远大于 1**
- 特征范数在深层 **指数放大**（论文报告可达 **~3000×** 量级）
- 训练在万步级 **发散**
...
详见 [v4.md](./v4.md) 与 [演进总览 §3.7](../reports/deepseek-version-lineage-20260625.md#37-deepseek-v4)。
...
| 方向 | 文档 |
|------|------|
| 前置概念 | [HC 基础](./hyper-connections.md) · [§3 双随机流形](#3-mhc-核心双随机流形约束) · [Raschka 全文 §8](../reports/raschka-technical-deepseek-v3-v32.md#8-appendix-mhcmanifold-constrained-hyper-connections) |
| 同代 V4 组件 | [csa-hca-mixed-attention.md](./csa-hca-mixed-attention.md) · [v4.md](./v4.md)（CSA、HCA、Muon、Hash MoE） |
| 对比（无 mHC 的上一档） | [v3-2.md](./v3-2.md) |
...
3. Raschka. *From DeepSeek V3 to V3.2* — [§8 mHC 附录](../reports/raschka-technical-deepseek-v3-v32.md#8-appendix-mhcmanifold-constrained-hyper-connections).
