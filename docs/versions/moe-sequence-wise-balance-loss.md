# MoE sequence-wise balance loss（$L_{\mathrm{Bal}}$）
...
| 方向 | 文档 |
|------|------|
| **本节点（④ $L_{\mathrm{Bal}}$）** | [MoE 线导读 §1](../reports/deepseek-moe-line.md#1-演进链ffn--路由) |
| **主机制 ③** | [aux-loss-free-moe-routing.md](./aux-loss-free-moe-routing.md) |
...
## 二、$P_i$：平均路由概率（Eq. 19–20）

> **核心**：$P_i$ = 这条序列上，router 对 expert $i$ 的 **归一化意愿** 的时间平均；均匀时 $P_i = 1/N_r$。

### 公式（三步）

**Step 1 — 原始 affinity**（每个 token、每个 expert）
...
```
前向：逐 token 算 s_{i,t} → s+b 选 top-K_r → expert 计算
      同时积累 count_i（算 f_i）和 s'_{i,t}（算 P_i）

聚合：f_i = (N_r / K_r T) · count_i
      P_i = (1/T) · Σ_t s'_{i,t}
      L_Bal = α · Σ_i f_i · P_i

反传：∂L_Bal/∂s_{i,t} → router 参数
      （b_i 仍由 step 末启发式单独更新，不走这项梯度）
```
...
**$\alpha$**：论文取 **极小值**（*extremely small*），$L_{\mathrm{Bal}}$ 只作安全网（对比 [aux-loss-free Table 5](./aux-loss-free-moe-routing.md#表v3-论文-table-5-摘要aux-loss-free-vs-纯-aux-loss-基线) 里大 $\alpha$ 的纯 aux-loss 基线）。
...
- [aux-loss-free-moe-routing.md](./aux-loss-free-moe-routing.md) — $b_i$、top-$K$ 双轨
