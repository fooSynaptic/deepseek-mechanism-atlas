...
**Hyper-Connections（HC）** 把 Transformer 每层 **单条残差流** 扩成 **$n$ 条并行残差流**，用可学习矩阵 **pre / post / comb** 在流与子层（Attention / FFN）之间混合；动机是 **加宽残差信息高速公路**，但无约束时 **破坏恒等映射** → 深层信号可 **指数放大**，需 [mHC](./mhc-manifold-hyper-connections.md) 的 **双随机流形** 约束后才可大规模训练。
...
| **下游（约束版）** | [mhc-manifold-hyper-connections.md](./mhc-manifold-hyper-connections.md)（含 [§3 双随机流形](./mhc-manifold-hyper-connections.md#3-mhc-核心双随机流形约束)） |
| **正交模块** | [CSA/HCA](./csa-hca-mixed-attention.md)（Attention / KV）· [MoE 线](../reports/deepseek-moe-line.md)（FFN 路由） |
...
[直接打开 SVG](../figures/mhc/hyper-connections.svg)
...
与 [mHC 主文档 §4](./mhc-manifold-hyper-connections.md#4-单层数据流推理) 及 vLLM `MHCPreOp` / `MHCPostOp` 对齐：
...
**mHC 的修复**（不在本文展开）：把上述混合矩阵 **投影到双随机流形** $\mathcal{B}_n$，使混合变为 **凸组合 / 加权平均** → 见 [mHC §3](./mhc-manifold-hyper-connections.md#3-mhc-核心双随机流形约束)。
...
| 文档 | **本文** | [mhc-manifold-hyper-connections.md](./mhc-manifold-hyper-connections.md) |
...
| 方向 | 文档 |
|------|------|
| **下游** | [mHC](./mhc-manifold-hyper-connections.md)（[§3 双随机流形](./mhc-manifold-hyper-connections.md#3-mhc-核心双随机流形约束)） |
| **同代 V4** | [v4.md](./v4.md) · [CSA/HCA](./csa-hca-mixed-attention.md) |
| **外部解读** | [Raschka §8](../reports/raschka-technical-deepseek-v3-v32.md#8-appendix-mhcmanifold-constrained-hyper-connections) |
