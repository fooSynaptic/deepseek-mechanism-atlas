...
> [← 中文导读](../README.md) · [← 仓库首页（EN）](../../README.md) · [← 演进总览 §3.1](../reports/deepseek-version-lineage-20260625.md#31-deepseek-llm-v1) · [← 版本目录](./README.md) · **[BBPE 词表专文](./v1-bbpe-tokenizer.md)** · [V1→V3 演进](../reports/deepseek-v1-to-v3-lineage.md)  
...
**Tokenizer** — 完整解读见 **[BBPE 词表与 Tokenizer 专文](./v1-bbpe-tokenizer.md)**（§2.1 论文段落；V2 沿用同词表）：

| 项 | 值 | 专文 |
|----|-----|------|
| 算法 | **[BBPE](./v1-bbpe-tokenizer.md#bbpe-overview)**（Byte-level BPE；[BPE 简述](./v1-bbpe-tokenizer.md#bpe-primer)） | HuggingFace [tokenizers](https://github.com/huggingface/tokenizers) |
| [预分词](./v1-bbpe-tokenizer.md#pre-tokenization) | 换行 / 标点 / CJK **不跨类 merge**（同 GPT-2） | 避免中英混排边界错误 |
| 数字 | 拆成 **单 digit**（同 LLaMA） | 算术、代码、编号更稳 |
| 常规 + special | **100,000 + 15 → 100,015** | [§4 词表规模](./v1-bbpe-tokenizer.md#vocab-size) |
| BBPE 训练语料 | ~**24GB** 多语言 | 来自上述三阶段管线子集 |
| **`vocab_size`（embedding）** | **102,400**（预留扩展） | 有效 id 仍以 100,015 为准；见专文 [易混点](./v1-bbpe-tokenizer.md#vocab-size) |

[编码流程（推理侧）](./v1-bbpe-tokenizer.md#encode-flow) · [系列延续（V2→V4）](./v1-bbpe-tokenizer.md#lineage)

<img src="../figures/v1/bbpe/bbpe-process-example.svg" alt="BBPE 训练过程示例：UTF-8 字节、迭代 merge、编码" width="920"/>

[直接打开 SVG](../figures/v1/bbpe/bbpe-process-example.svg) · 详述见 [BBPE 专文 §2](./v1-bbpe-tokenizer.md#bbpe-overview)
...
> 原理详解、公式与 LLaMA cosine 对比 → [学习率调度 Wiki](./v1.md#训练)
...
| $n_{\mathrm{vocab}}$ | 词表大小（V1 配置 **102,400**，见 [BBPE 专文 §4](./v1-bbpe-tokenizer.md#vocab-size)） |
| $l_{\mathrm{seq}}$ | 序列长度 |
...
- [版本演进总览](../reports/deepseek-version-lineage-20260625.md) · [《ds-技术报告》§3.1 V1](../../《ds-技术报告》/01-总览/01-版本演进总览.md#31-deepseek-llm-v1)
- **[BBPE 词表与 Tokenizer 专文](./v1-bbpe-tokenizer.md)**（§2.1 Tokenizer；预分词 / 102,400 embedding）
- [V1→V3 演进](../reports/deepseek-v1-to-v3-lineage.md)
- **Scaling 专题（本仓答疑）**
  ...
  - [学习率调度（§2.3 multi-step vs cosine）](./v1.md#训练)
- [中文结构化译文](./v1-technical-report.zh.md)
- [技术要点提炼](./v1-bbpe-tokenizer.md)
- [索引](./v1.md)
