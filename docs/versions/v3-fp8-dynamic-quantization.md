...
> **答疑**：[partial sum 漂移是什么、为何每 128 MMA 要提升 FP32？](./qa/fp8-partial-sum-drift.md)（含 (a)/(b) **实现分工表**）
...
这与 V3 报告里 **FP8 训练框架**（配合 DualPipe 并行、DeepEP 通信）同属 **训推系统层**；换推理引擎 FP8 kernel（如自测 [draft+FP8](./dspark-speculative-decoding.md#3-外挂-draft-自测qwen-4b--06b)）是 **另一套部署量化**，数值策略不必与训练块 scale 完全相同。
