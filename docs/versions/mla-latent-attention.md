...
[图示详情](../figures/mla/mla-forward-flow.svg)
...
> V3.1 **Hybrid**：Prefill 时 Q/K[可按 **MHA 式**展开 latent；Decode 时 **MQA 式**共享 latent](./v3-1.md#mla-模式切换terminus-起)。**cache 布局仍是 MLA latent**，切换的是展开方式。
...
| **下游 ② 异构 cache** | [DSA稀疏注意力§异构 KV](./dsa-sparse-attention.md#异构-kv-cache) |
...
| **下游 ② DSA** | [DSA 稀疏注意力](./dsa-sparse-attention.md)（MLA 结构不变，加稀疏选择） |
| **下游 ③ CSA/HCA** | [CSA / HCA](./csa-hca-mixed-attention.md) · [DeepSeek-V4](./v4.md)（新注意力，不再单一 MLA latent） |
...
| **V3.2 + DSA** | MLA **不变**；[indexer 在 latent 序列上选 top-$k$](./dsa-sparse-attention.md)、[Core 仍做 MLA](./dsa-sparse-attention.md) |
| **V4** | CSA/HCA 等 **[新注意力](./csa-hca-mixed-attention.md)**，不再单一 MLA latent · [DeepSeek-V4](./v4.md) |
...
| 资源 | 说明 |
|------|------|
| [MLA前向计算流程（含 PyTorch 对照）](./mla-latent-attention.md) | 更长的 shape 推演与 `mla_forward.py` |
| [FlashMLA](https://github.com/deepseek-ai/FlashMLA) | V3+ 推理 kernel |
| [DeepSeek-V3](./v3.md) | V3 梗概中的 MLA 一行 |
