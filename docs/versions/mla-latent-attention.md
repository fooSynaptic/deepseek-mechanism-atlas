# MLA（Multi-Head Latent Attention）前向计算流程
...
<img src="../figures/mla/mla-forward-flow.svg" alt="MLA 前向计算流程（公式 37–47）：Query / Key / Value 三分支、KV Cache 压缩对比（DeepSeek-V2 维数标注）" width="920"/>

[直接打开 SVG](../figures/mla/mla-forward-flow.svg)
...
> V3.1 **Hybrid**：Prefill 时 Q/K 可按 **MHA 式**展开 latent；Decode 时 **MQA 式**共享 latent（见 [v3-1.md §MLA 模式切换](./v3-1.md#mla-模式切换terminus-起)）。**cache 布局仍是 MLA latent**，切换的是展开方式。
...
| 方向 | 文档 |
|------|------|
| **本节点（① 同质 MLA KV）** | [基础设施线导读 §1](../reports/deepseek-infra-line.md#1-演进链kv--offload) |
| **下游 ② 异构 cache** | [dsa-sparse-attention.md §异构 KV](./dsa-sparse-attention.md#异构-kv-cache) |
...
| 方向 | 文档 |
|------|------|
| **本节点（① MLA）** | [算法线导读 §1](../reports/deepseek-algorithm-line.md#1-演进链attention--残差) |
| **下游 ② DSA** | [dsa-sparse-attention.md](./dsa-sparse-attention.md)（MLA 结构不变，加稀疏选择） |
| **下游 ③ CSA/HCA** | [csa-hca-mixed-attention.md](./csa-hca-mixed-attention.md) · [v4.md](./v4.md)（新注意力，不再单一 MLA latent） |
...
| **V3.2 + DSA** | MLA **不变**；indexer 在 latent 序列上选 top-$k$，Core 仍做 MLA（见 [dsa-sparse-attention.md](./dsa-sparse-attention.md)） |
| **V4** | CSA/HCA 等 **新注意力**，不再单一 MLA latent（见 [csa-hca-mixed-attention.md](./csa-hca-mixed-attention.md) · [v4.md](./v4.md)） |
...
| 资源 | 说明 |
|------|------|
| [mla-详解.md（含 PyTorch 对照）](./mla-latent-attention.md) | 更长的 shape 推演与 `mla_forward.py` |
| [FlashMLA](https://github.com/deepseek-ai/FlashMLA) | V3+ 推理 kernel |
| [v3.md](./v3.md) | V3 梗概中的 MLA 一行 |
