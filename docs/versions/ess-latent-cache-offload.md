...
- 论文梗概见 [ess-paper-highlights](ess-paper-highlights.md)。
...
**归属**：ESS 为 **百度百舸（Baige AI）** 提出（[arXiv:2512.10576](https://arxiv.org/abs/2512.10576)），优化 **Latent-Cache 显存**；与 **清华 + 智谱** 的 [Index Share / IndexCache](./index-share.md)（优化 **indexer 计算**）**正交**，可同开。
...
Indexer 回答「**看哪 2048 个位置**」；Latent-Cache 回答「**这些位置的 MLA K/V 是多少、怎么加权**」。详见 [MLA 前向流程图](./mla-latent-attention.md#forward-flow)。
...
DSA 异构 cache 同时支撑 **ESS（搬 latent）** 与 **[Index Share](./index-share.md)（省 indexer 计算）**。
