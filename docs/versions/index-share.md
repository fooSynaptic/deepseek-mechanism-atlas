...
> [← 中文导读](../README.md) · [← 仓库首页（EN）](../../README.md) · [← 演进总览 §4](../reports/deepseek-version-lineage-20260625.md#4-index-shareindexcache) · [← 基础设施线导读](../reports/deepseek-infra-line.md) · [← 版本目录](./README.md) · [上游 DSA §异构 KV](./dsa-sparse-attention.md#异构-kv-cache) · [并列 ESS](./ess-latent-cache-offload.md) · [逻辑详解](../dsa/index-share-logic.md)
...
- 逻辑详解见 [Index Share 系列](../dsa/index-share-logic.md)。

---

## 定位

社区昵称 **Index Share** / 「**V3.3**」；正式名 **IndexCache**（清华 + Z.ai，2026-03）。**不是新模型**，而是面向 **[DSA](./dsa-sparse-attention.md)** 架构的 **纯推理 infra 补丁**：零额外显存，在 V3.2 / GLM-5 等模型上即插即用。

典型体现「**infra 归 infra，算法归算法**」——算法仍是 V3.2 的 **[DSA](./dsa-sparse-attention.md)**，系统侧利用跨层冗余砍掉冗余 indexer 计算。

> **逻辑详解**：[Index Share逻辑详解](../dsa/index-share-logic.md) · [上游 DSA §异构 KV](./dsa-sparse-attention.md#异构-kv-cache)
...
| 方向 | 文档 |
|------|------|
| **本节点（③ Index Share）** | [基础设施线导读 §1](../reports/deepseek-infra-line.md#1-演进链kv--offload) |
| **前置 ② 异构 cache** | [DSA稀疏注意力§异构 KV](./dsa-sparse-attention.md#异构-kv-cache) |
| **并列 ④ ESS** | [ESS Latent offload](./ess-latent-cache-offload.md)（Latent offload，可同开） |
| **下游 ⑤ V4** | [CSA / HCA](./csa-hca-mixed-attention.md) · [DeepSeek-V4](./v4.md)（V4 自带 CSA indexer，路线互补） |
...
| 4 | **百度百舸（Baige AI）** 在 IndexCache 上主要是**工程集成与云侧落地**；其**自研**的同类 infra 是 **[ESS](./ess-latent-cache-offload.md)**（Latent-Cache offload），与 IndexCache **正交** |
...
> **逻辑详解**：[Index Share逻辑详解](../dsa/index-share-logic.md) §1

---

[DSA](./dsa-sparse-attention.md) 每层独立跑 lightning indexer，复杂度 $O(L^2)$。长上下文 prefill 时 indexer 成为显著瓶颈。观察：**相邻层选出的 top-$k$ index 高度相似**。

## 机制

> **逻辑详解**：[Index Share逻辑详解](../dsa/index-share-logic.md) · [DSA 前置](./dsa-sparse-attention.md)
...
- **零额外 GPU 显存**
- 仅适用于 **[DSA](./dsa-sparse-attention.md) 系**（DeepSeek-V3.2、GLM-5 等）
- 与 **[ESS](./ess-latent-cache-offload.md)** Latent-Cache offload 正交，可叠加
