...
|------|----------------|------------------|
| Cache 结构 | 同质 Latent-Cache（per-token MLA） | **CSA/HCA/SWA/Indexer/tail** 异构（见 [KV layout](./v4-kv-layout.md)） |
| Offload 粒度 | 按 **latent entry**；依赖 DSA top-$k$ **时间局部性** | 按 **C4 压缩 entry**；依赖 CSA **空间稀疏激活** |
| Indexer | GPU 常驻 | V4 indexer 仍参与 CSA 路径；layout 见 [v4.md](./v4.md) |
...
[直接打开 SVG](../figures/v4/v4-hetero-kv.svg) · 图下半区标注 HiSparse offload
...
| **并列 prefix** | [v4-disk-prefix-cache.md](./v4-disk-prefix-cache.md) |
| **上游 ④ ESS** | [ess-latent-cache-offload.md](./ess-latent-cache-offload.md)（**非**简单放大） |
