...
**DeepSeekMoE** = DeepSeek 自研的 **细粒度 routed experts + shared experts** FFN 稀疏激活框架：每 token 只激活少量 routed expert，并 **始终保留 shared expert** 路径；V2（2024-05）**首次引入**，V3 / R1 / V3.1 / V3.2 **沿用同一 MoE 骨架**，V3 起路由改为 [aux-loss-free](./aux-loss-free-moe-routing.md)，V4 前几层改 **[Hash 路由](./v4.md)**。
...
V2 论文相对 67B 稠密：训练成本 **-42.5%**、生成吞吐 **5.76×**（同参数量级对比语境见 [DeepSeek-V2](./v2.md)）。
...
> **版本文档**：[V2](./v2.md) · [V3](./v3.md) · [V4](./v4.md)
...
| **上游 ①** | 稠密 FFN（概念阶段；V1 产品实例见 [DeepSeek-LLM V1](./v1.md)） |
| **下游 ③④** | [aux-loss-free MoE 路由](./aux-loss-free-moe-routing.md) · [序列均衡损失](./moe-sequence-wise-balance-loss.md) |
| **下游 ⑤** | [Hash MoE + FP4](./hash-moe-fp4.md) · [DeepSeek-V4](./v4.md) |
