...
> [← 中文导读](../README.md) · [← 仓库首页（EN）](../../README.md) · [← 演进总览 §2](../reports/deepseek-version-lineage-20260625.md#2-版本时间线与关系) · [← MoE 线导读](../reports/deepseek-moe-line.md) · [V2 首发版本](./v2.md) · [V3 旗舰化](./v3.md) · [aux-loss-free 下游](./aux-loss-free-moe-routing.md)  
...
**DeepSeekMoE** = DeepSeek 自研的 **细粒度 routed experts + shared experts** FFN 稀疏激活框架：每 token 只激活少量 routed expert，并 **始终保留 shared expert** 路径；V2（2024-05）**首次引入**，V3 / R1 / V3.1 / V3.2 **沿用同一 MoE 骨架**，V3 起路由改为 [aux-loss-free](./aux-loss-free-moe-routing.md)，V4 前几层改 **Hash 路由**（见 [v4.md](./v4.md)）。
...
V2 论文相对 67B 稠密：训练成本 **-42.5%**、生成吞吐 **5.76×**（同参数量级对比语境见 [v2.md](./v2.md)）。

---

<a id="optimization-logic"></a>

## 优化逻辑：从常规 MoE 到 DeepSeekMoE（V2 论文 Figure 2 示意）
...
### (b) + 细粒度专家切分（Fine-grained Segmentation）
...
> **答疑**：[Fine-grained 为何优于 GShard？](./qa/moe-fine-grained-segmentation.md) — 组合数 $\binom{N}{K}\!\to\!\binom{mN}{mK}$（论文例 120 → $4.4\times 10^9$）、IsoFLOP 切分与训练机制

### (c) + Shared Expert 隔离（DeepSeekMoE 完整形态）
...
| $e_i$ | 第 $i$ 个 routed expert 的 routing **centroid**（非 FFN 权重；见 [答疑：centroid vs gate-weight](./qa/moe-centroid-vs-gate-weight.md)） |
...
| 负载均衡 | **Expert / device / comm** 三级 aux loss（V3 起路由改为 [aux-loss-free](./aux-loss-free-moe-routing.md)，MoE **骨架**仍为 shared + fine-grained routed） |
...
4. **路由演进**：V2 **softmax** + aux balance loss → V3 [aux-loss-free](./aux-loss-free-moe-routing.md) + [$L_{\mathrm{Bal}}$](./moe-sequence-wise-balance-loss.md)。
...
| **V3** | 671B / **37B** | **256** routed / 层 | **8** | 有 | [aux-loss-free](./aux-loss-free-moe-routing.md) |
| **V4** | 1.6T / 49B 等 | 继承 V3 框架 | 同族 | 有 | 前几层 **Hash MoE** + FP4 |

> **版本文档**（训练规模、上下文、后训练）：[V2](./v2.md) · [V3](./v3.md) · [V4](./v4.md)
...
| 方向 | 文档 |
|------|------|
| **本节点（② DeepSeekMoE）** | [MoE 线导读 §1](../reports/deepseek-moe-line.md#1-演进链ffn--路由) |
| **上游 ①** | 稠密 FFN（概念阶段；V1 产品实例见 [v1.md](./v1.md)） |
| **下游 ③④** | [aux-loss-free-moe-routing.md](./aux-loss-free-moe-routing.md) · [moe-sequence-wise-balance-loss.md](./moe-sequence-wise-balance-loss.md) |
| **下游 ⑤** | [hash-moe-fp4.md](./hash-moe-fp4.md) · [v4.md](./v4.md) |
...
- [DeepSeek-V2 梗概](./v2.md) — MLA + DeepSeekMoE **首次落地**（8.1T）
- [DeepSeek-V3 梗概](./v3.md) — 256/8 旗舰化 + MTP
- [aux-loss-free 路由逻辑](./aux-loss-free-moe-routing.md)
