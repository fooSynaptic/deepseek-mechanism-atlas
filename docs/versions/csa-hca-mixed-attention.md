...
> [← 中文导读](../README.md) · [← 仓库首页（EN）](../../README.md) · [← 演进总览 §3.7](../reports/deepseek-version-lineage-20260625.md#37-deepseek-v4) · [← 算法线导读](../reports/deepseek-algorithm-line.md) · [← 基础设施线导读](../reports/deepseek-infra-line.md) · [V4 梗概](./v4.md) · [上游 DSA](./dsa-sparse-attention.md) · [上游 MLA](./mla-latent-attention.md) · [并列 mHC](./mhc-manifold-hyper-connections.md) · [Lightning Indexer 详解](../dsa/lightning-indexer.md)  
...
| 方向 | 文档 |
|------|------|
| **本节点（③ CSA / HCA）** | [算法线导读 §1](../reports/deepseek-algorithm-line.md#1-演进链attention--残差) |
| **上游 ② DSA** | [dsa-sparse-attention.md](./dsa-sparse-attention.md) · [dsa-logic.md](../dsa/dsa-logic.md) |
| **上游 ① MLA** | [mla-latent-attention.md](./mla-latent-attention.md)（V4 不再单一 per-token latent） |
| **同代 V4 其他组件** | [v4.md](./v4.md)（两个规格、MoE、Muon） · [mHC](./mhc-manifold-hyper-connections.md)（残差路径，与 Attention **正交**） |
| **infra 依赖** | [KV layout](./v4-kv-layout.md) · [HiSparse](./v4-hisparse.md) · [磁盘 prefix](./v4-disk-prefix-cache.md) |
...
2. 当前 query 经 **lightning indexer**（思想延续 [DSA](./dsa-sparse-attention.md) / [Lightning Indexer](../dsa/lightning-indexer.md)）在 **$\sim L/4$ 条压缩 entry** 上打分。
...
V4 **不是**「全层 CSA 或全层 HCA」二选一，而是在同一模型中 **组合多种 attention 路径**（论文与部署文档中的 **异构 KV**）：
...
**内存布局**（Classical + State 双池、$\mathrm{lcm}(4,128)$ 块对齐）→ **[V4 KV Layout 专文](./v4-kv-layout.md)**，算法对象与引擎池化的分界见该文 §1。

---

## 5. 相对 V3.2 的效率（论文 Figure 1）

V4 把 **算力** 与 **KV 体积** 同时压下来，CSA/HCA 是算法侧主因；infra 侧另需 [HiSparse](./v4-hisparse.md) / [磁盘 prefix](./v4-disk-prefix-cache.md) 才能「跑得动」1M。
...
详见 [v4.md §1M context 效率](./v4.md#1m-context-效率相对-v32)。

---

## 6. 训练侧：渐进式上下文

V4 训练采用 **渐进拉长上下文**（[v4.md §训练要点](./v4.md#训练要点)）：
...
|------------|---------------------|
| [KV layout](./v4-kv-layout.md) | Classical 池存 CSA/HCA 压缩块；State 池存 tail + SWA |
| [HiSparse](./v4-hisparse.md) | 针对 **C4 inactive entry** 的 CPU offload |
| [磁盘 prefix cache](./v4-disk-prefix-cache.md) | CSA/HCA 压缩 entry **可直接落盘**；SWA 三档策略另计 |
| [ESS](./ess-latent-cache-offload.md) | **不可直迁** — ESS 面向 V3.2 per-token latent |

完整 **基础设施线** 见 [deepseek-infra-line.md](../reports/deepseek-infra-line.md#1-演进链kv--offload)。
...
MLA（per-token latent KV）
  → DSA（latent 上 indexer + top-k 稀疏）
    → CSA（4:1 块压缩 + 压缩序列上 top-k）
...
| 方向 | 文档 |
|------|------|
| 版本总览 | [v4.md](./v4.md)（两个规格、MoE、Muon、训练要点） |
| 上游 | [v3-2.md](./v3-2.md) · [dsa-sparse-attention.md](./dsa-sparse-attention.md) |
| 并行（V3.2 infra） | [index-share.md](./index-share.md) — 纯 infra，与 V4 路线 **互补** |
| 同代残差 | [mhc-manifold-hyper-connections.md](./mhc-manifold-hyper-connections.md) |
