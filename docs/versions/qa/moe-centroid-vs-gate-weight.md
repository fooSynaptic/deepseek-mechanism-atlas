...
DeepSeekMoE 把 $N$ 个大 expert 拆成 $2N$ 个小 expert（见 [Figure 2 优化逻辑](../deepseek-moe.md#optimization-logic)）：
...
V4 前几层改用 **Hash 路由**（见 [hash-moe-fp4.md](../hash-moe-fp4.md)）— 不再用 $u^\top e_i$ 做语义匹配，而是确定性 hash。
