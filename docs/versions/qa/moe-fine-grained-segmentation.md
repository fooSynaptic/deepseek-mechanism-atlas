...
[← 返回 DeepSeekMoE §优化逻辑 (b)](../deepseek-moe.md#optimization-logic) · [答疑目录](./README.md)
...
Fine-grained 的操作（论文 §3.1）：

1. 每个标准 expert FFN **切成 $m$ 个小 expert**；
2. 中间维缩为 **$1/m$**；
3. 每 token 激活数 **$K \to mK$**。

**总参 / 激活 FLOPs 不变**（与 [Figure 2 (b)](../deepseek-moe.md#optimization-logic) 一致）：
...
## 3. GShard 式粗 expert 的特化问题（补充视角）

组合空间小 **且** 单 expert 太大时，还会出现 **知识重叠**（论文 §3.1）：
...
| [Shared isolation](../deepseek-moe.md#optimization-logic) | 通用知识 **不占用** routed 组合槽位 |
| [Centroid affinity](../qa/moe-centroid-vs-gate-weight.md) | 在巨大组合空间中 **按簇** 选 expert，避免随机乱配 |
...
| V2 aux loss / V3 [aux-loss-free](../aux-loss-free-moe-routing.md) | 防负载塌缩 |
...
**Fine-grained 在 IsoFLOP 下把 $\binom{N}{K}$ 换成 $\binom{mN}{mK}$**（论文例：120 → $4.4\times 10^9$），让 router 对每个 token 能从 ** vastly larger 的 specialist 子集菜单** 里精确配知识；再配合更窄的单 expert 与 shared/路由设计，**同等算力下训练效果更好**。DeepSeek MoE 线（V2→V3→V4）均建立在这一组合数学优势之上。
