...
**实现上两路在同一次 GEMM 内核里串联**：先 (a) 按块量化并发起 MMA，MMA 流水里嵌入 (b) 的 **128 步 promotion**；[反量化与 FP32 累加都在 **CUDA Core** 完成](../v3-fp8-dynamic-quantization.md)、[Tensor Core 只负责 FP8 乘加吞吐](../v3-fp8-dynamic-quantization.md)。
