# 投机解码：为何接受率是 $\min(1,p/q)$、修正分布是 $\max(0,p-q)$

[← 返回投机解码专文 §1.3](../dspark-speculative-decoding.md#13-净收益lossless-与最坏情况) · [答疑目录](./README.md)
...
## 3. 接受率为何是 $\min\!\left(1,\dfrac{p(x)}{q(x)}\right)$

记接受概率为 $\alpha(x)$。由 §2.3，**接受路径**上输出 $x$ 的概率为 $q(x)\,\alpha(x)$。我们希望这条路径对 $x$ 的贡献是 $p(x)$ 的一部分，且**不超过** $p(x)$（超出部分留给拒绝路径）。最自然要求是
...
> **基础恒等式（单个 $x$ 上恒成立）** — 记 $a=p(x)$、$b=q(x)$，则
>
> $$\min(a,b) + \max(0,\, a-b) = a.$$
>
...
- 专文机制总览：[dspark-speculative-decoding.md §1.2–§1.3](../dspark-speculative-decoding.md#12-一轮怎么走draft--verify--accept)
