...
> [← 中文导读](../README.md) · [← 仓库首页（EN）](../../README.md) · [← 版本目录](./README.md) · [← 演进总览](../reports/deepseek-version-lineage-20260625.md) · [← 基础设施线](../reports/deepseek-infra-line.md) · [← V3.1 Agent](./v3-1.md) · [← V4 长上下文 Agent](./v4.md) · [← DSpark](./dspark-speculative-decoding.md) · [← GRPO 长程局限](../rl/optimize.md)
...
本仓库原先用三条线组织 DeepSeek 开源主线（见 [演进总览 §1](../reports/deepseek-version-lineage-20260625.md#1-总览)）：
...
| 仓库节点                                                 | 与 Harness 的关系                                                                                     |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| [V3.1](./v3-1.md) Hybrid / Agent · Tool Use          | 后训练把 BrowseComp / SWE 等 **agent 能力**做强；Harness 是这类能力的 **产品侧载体，也是出口。**                             |
| [V3.2](./v3-2.md) / [DSA](./dsa-sparse-attention.md) | 长上下文 + 工具轨迹更便宜；Harness session 会吃更长的 tool/trace                                                   |
| [V4](./v4.md) Agentic Coding（100K–1M）                | V4 面向长程 agent 工作流；官方Harness 提供 **多步 tool / subagent / workflow** 的运行时，CSA/HCA 提供 **扛得住的 context** |
| [R1](./r1.md) / [RLVR](./rlvr.md)                    | 短程可验证推理；Harness 场景更接近 **长程 coding agent**                                                         |
| [GRPO 长程局限](../rl/optimize.md)                       | GRPO 舒适区是短程；长程 agent 要另谈吞吐与奖励——Harness 解决的是 **怎么跑与怎么观**；RL 算法见 R1 / RLVR                          |
...
| 仓库节点                                                                     | 对 agent 会话的意义                            |
| ------------------------------------------------------------------------ | ---------------------------------------- |
| [DSpark](./dspark-speculative-decoding.md)                               | 降 **decode** 延迟 → 多轮 tool loop 更快        |
| [V4 HiSparse](./v4-hisparse.md) · [磁盘 Prefix](./v4-disk-prefix-cache.md) | 超长 agent trace / 代码库前缀更扛得住               |
| [Index Share](./index-share.md) · [ESS](./ess-latent-cache-offload.md)   | 服务侧省 indexer 算力 / Latent 显存 → 同卡可撑更长会话   |
| [MTP](./v3.md#三mtpmulti-token-prediction)                                | 训练侧多 token；推理可接投机——与 Harness 的 tool 编排正交 |
...
| 主题                                          | 为何正交                                      |
| ------------------------------------------- | ----------------------------------------- |
| [Engram](../engram/README.md)               | 模型内部 **条件记忆 / 查表稀疏轴**；Harness 是进程外工具与会话框架 |
| [DeepSeekMoE](./deepseek-moe.md) / Hash MoE | 权重稀疏；Harness 走 LLM adapter 调用已部署模型 |
