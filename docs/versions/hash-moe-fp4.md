...
| **上游 ②–④** | [DeepSeekMoE](./deepseek-moe.md) · [aux-loss-free MoE 路由](./aux-loss-free-moe-routing.md) · [序列均衡损失](./moe-sequence-wise-balance-loss.md) |
| **版本总览** | [DeepSeek-V4](./v4.md)（两个规格、Attention、训练） |
...
DeepSeekMoE（V2→V3）用 **可学习 expert 向量** $e_i$ 与 token hidden $u$ 做亲和度 $u^\top e_i$，再 sigmoid + top-$K_r$ + 动态 bias $b_i$（[aux-loss-free](./aux-loss-free-moe-routing.md)）。语义上是 **[「token 匹配 expert 原型」](./qa/moe-centroid-vs-gate-weight.md)**。
...
- **DeepSeekMoE 形态**：细粒度 routed pool + **shared experts** 恒激活（[DeepSeekMoE](./deepseek-moe.md)）
...
MoE 改动与 CSA/HCA、mHC、Muon **同期打包**进 V4，**[难以单独 ablation](./v4.md#定位)** Hash vs FP4 vs Attention。
...
| 版本总览 | [DeepSeek-V4](./v4.md) |
| 路由对照 | [MoE 路由：gate-weight 还是 expert centroid？](./qa/moe-centroid-vs-gate-weight.md) §4.4 |
| FP8 前代 | [V3 FP8 动态量化](./v3-fp8-dynamic-quantization.md) |
