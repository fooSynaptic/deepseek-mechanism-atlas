# V4 KV Layout：Classical + State 双池（论文 §3.5.1）

> [← 中文导读](../README.md) · [← 仓库首页（EN）](../../README.md) · [← 演进总览 §5.3 KV layout](../reports/deepseek-version-lineage-20260625.md#v4-kv-layout) · [← 基础设施线导读](../reports/deepseek-infra-line.md) · [CSA/HCA 算法专文](./csa-hca-mixed-attention.md) · [V4 梗概 §推理 infra](./v4.md#推理-infra-关注点) · [并列 HiSparse](./v4-hisparse.md) · [并列 磁盘 Prefix Cache](./v4-disk-prefix-cache.md) · [上游 ESS](./ess-latent-cache-offload.md)  
...
算法侧五类 entry 见 [CSA/HCA 专文 §4](./csa-hca-mixed-attention.md#v4-mixed-attention) · [V4 梗概](./v4.md#推理-infra-关注点) 与 [演进总览 §5.3](../reports/deepseek-version-lineage-20260625.md#53-v4异构-kv--hisparse--磁盘-prefix-cache) 表格；**本文只讲引擎如何把它们落进内存**。

<img src="../figures/v4/v4-hetero-kv.svg" alt="V4 异构 KV：Classical KV cache + per-request state blocks (paper sec 3.5.1)" width="920"/>

[直接打开 SVG](../figures/v4/v4-hetero-kv.svg)
...
| **生命周期** | 随 prefix 增长 append；压缩完成后 entry **不可变**，适合 **prefix 共享与落盘**（见 [磁盘 Prefix Cache](./v4-disk-prefix-cache.md)） |
...
论文 §3.5.1 的 **State cache** 是 SWA + tail 的归宿；部署上 SWA 往往占 **per-token 体积大头**（Together 早期 bring-up：全量 SWA 时 per-token KV 甚至略高于 V3 路径），因此 cache **策略**（存多少、何时 evict）比压缩算法本身更决定并发容量——见 [HiSparse 专文](./v4-hisparse.md)。
...
> V4 **不是**把 ESS 的 Latent-Cache 直接放大到 1M；而是 **换一套异构压缩 layout** 后再做分层内存（[ESS §与 V4](./ess-latent-cache-offload.md#与-index-share--v4-的关系)）。
...
| 方向 | 文档 |
|------|------|
| **本专题（layout 基础）** | [演进总览 §5.3](../reports/deepseek-version-lineage-20260625.md#v4-kv-layout) |
| **infra 线 ⑤ 子项** | [基础设施线导读 §1](../reports/deepseek-infra-line.md#1-演进链kv--offload) |
| **并列 offload** | [HiSparse](./v4-hisparse.md) · [磁盘 Prefix Cache](./v4-disk-prefix-cache.md) |
| **上游 ④ ESS** | [ess-latent-cache-offload.md](./ess-latent-cache-offload.md) |
| **算法依赖** | [csa-hca-mixed-attention.md](./csa-hca-mixed-attention.md) · [V4 梗概](./v4.md) · [算法线 §③](../reports/deepseek-algorithm-line.md#1-演进链attention--残差) |
...
| 资源 | 说明 |
|------|------|
| [演进总览 §5.4](../reports/deepseek-version-lineage-20260625.md#54-三代-offload-对比) | V3 / V3.2 ESS / V4 三代 offload 对照 |
...
| [v4-hetero-kv.svg](../figures/v4/v4-hetero-kv.svg) | Classical + HiSparse 分层示意图 |
