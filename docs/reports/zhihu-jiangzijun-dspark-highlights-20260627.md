...
> [← 返回 V3 §三 MTP](../versions/v3.md#三mtp-multi-token-predictionv3-新增顶层结构) · [投机解码专文](../versions/dspark-speculative-decoding.md) · [报告目录](./README.md)
...
**对照**：[DeepSpec](https://github.com/deepseek-ai/DeepSpec) · [DSpark 论文 PDF](https://github.com/deepseek-ai/DeepSpec/blob/main/DSpark_paper.pdf) · [投机解码专文](../versions/dspark-speculative-decoding.md)
...
> **Compute / Memory 两层读法 · DFlash vs Eagle**：[答疑：draft 侧 DFlash≈并行 Compute-Bound、Eagle≈串行 Memory-Bound](../versions/qa/spec-decode-compute-vs-memory-bound.md) — 开篇整句指 **target 并行 verify**；draft 范式上 **DFlash / Eagle** 分别对应两侧。
...
加速比粗式（见 [专文 §4](../versions/dspark-speculative-decoding.md#4-草稿范式总览eagle3--dflash--mtp--dspark)）：
...
<img src="../versions/figures/dspark-semi-ar-draft.svg" alt="DSpark 半自回归：并行主干一次猜 K 位，顺序头逐位补因果" width="920"/>

[直接打开 SVG](../versions/figures/dspark-semi-ar-draft.svg)
...
| **draft 侧算力** | **几乎不减**。并行主干往往仍生成完整 $K$ 块草稿；截断只省 **verify 侧**（见 [专文 §11](../versions/dspark-speculative-decoding.md#11-局限与边界)）。 |
...
**负载自适应**（专文 §6.2）：低并发可验 4–6 位；高并发 **平滑缩短** $L_j$。

<img src="../versions/figures/dspark-confidence-scheduler.svg" alt="置信度调度：校准、前缀调度、负载自适应截断" width="920"/>

[直接打开 SVG](../versions/figures/dspark-confidence-scheduler.svg)
...
> **读图**：[MTP 融合 scheme 专文](../versions/qa/mtp-fusion-scheme.md) · [融合 SVG](../versions/figures/mtp-fusion-scheme.svg) · [串行链深度计算图](../versions/figures/mtp-draft-chain-depth.svg)
...
<img src="../versions/figures/mtp-fusion-scheme.svg" alt="MTP 中间 token 融合：主网 h + Emb(x_{t+k}) -> MTPBlock_k -> 预测 x_{t+k+1}" width="920"/>

[直接打开 SVG](../versions/figures/mtp-fusion-scheme.svg) · [投机对照总览（旧图）](../versions/figures/mtp-speculative.svg)

<img src="../versions/figures/mtp-draft-chain-depth.svg" alt="MTP draft 串行链：共享 OutHead，链深度 0/1/2 逐步 Emb 注入" width="920"/>

[直接打开链深度 SVG](../versions/figures/mtp-draft-chain-depth.svg) · [专文 §1.1 理解对照](../versions/qa/mtp-fusion-scheme.md#11-你的理解对在哪里错在哪里)

---

## DeepSpec draft 训练 vs 主模型 fine-tune

> **范畴**（[专文 §9](../versions/dspark-speculative-decoding.md#9-训练与在线引擎)）：**DSpark 线上推理**不改 V4 基座权重。**「训练」**指 DeepSpec 里 **外挂 draft** 的蒸馏/SFT；**「在线引擎」**指 draft 接入后的调度与 kernel。**二者都不是** V3/V4 **主模型预训练或 fine-tune**。
...
```
离线：新样本 →（可选）SFT target → 改能力 p
              ↓ 并行
        蒸馏 draft（target 冻结）→ 让 q 贴近 p

在线：draft 猜 + target verify → 加速（输出仍服从 p）
```
...
| 主题 | 专文章节 |
|------|----------|
| Compute / Memory；DFlash vs Eagle | [答疑 §1–§7](../versions/qa/spec-decode-compute-vs-memory-bound.md) |
| 投机循环、lossless | [§1](../versions/dspark-speculative-decoding.md#1-投机解码循环) |
| MTP vs 外挂 draft | [§2](../versions/dspark-speculative-decoding.md#2-deepseek-路线mtpv3--v4) |
| MTP 中间 token 融合 | [mtp-fusion-scheme.md](../versions/qa/mtp-fusion-scheme.md) · [SVG](../versions/figures/mtp-fusion-scheme.svg) |
| Eagle / DFlash / DSpark 范式 | [§4](../versions/dspark-speculative-decoding.md#4-草稿范式总览eagle3--dflash--mtp--dspark) |
| 机制细节 | [§5–§6](../versions/dspark-speculative-decoding.md#5-dspark-概述) |
| 线上 MTP-1 基线 | [§8、§10](../versions/dspark-speculative-decoding.md#8-v4-生产部署) |
