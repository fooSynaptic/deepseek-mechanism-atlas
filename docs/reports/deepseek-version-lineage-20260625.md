...
| **[DeepSeek-V4.1-Flash](../versions/v4-1-flash.md)** | 2026-09 | 552B | **8B / 16B** | **1M** | DeepSeek | [HF Tech Report](https://huggingface.co/deepseek-ai/DeepSeek-V4.1-Flash/blob/main/DeepSeek_V41_Tech_Report.pdf) | **CED** + **CSA2** + FP4 KV；prefill/decode 分激活 |
...
### 3.7b [DeepSeek-V4.1-Flash](../versions/v4-1-flash.md)

<a id="37b-deepseek-v41-flash"></a>

> **梗概**：[DeepSeek-V4.1-Flash](../versions/v4-1-flash.md)

**官方材料**：[HF 技术报告 PDF](https://huggingface.co/deepseek-ai/DeepSeek-V4.1-Flash/blob/main/DeepSeek_V41_Tech_Report.pdf) · [模型卡](https://huggingface.co/deepseek-ai/DeepSeek-V4.1-Flash)

**模型**：骨干 **552B**；激活 **8B（prefill）/ 16B（decode）**；Engram **196B**；上下文至 **1M**；原生多模态。

**相对 V4-Flash 的下一跳**

| 组件 | 说明 |
|------|------|
| **CED** | 40 层 = 因果 encoder(20) + decoder(20)；decoder 全局 KV 由 \(H_{20}\) 投影 → prefill 约半深 |
| **CSA2** | Full / Reindex / Reuse 三模式；indexer K 由 main KV 投影；解码侧 Hierarchical Sparse Indexer |
| **FP4 main KV** | 与 CSA2 叠乘 → 全局 KV ≈ **890 B/token**（约 V4-Flash 的 1/4） |
| **SWA Bounded Replay** | 持久化 cache 不再存 SWA；未命中时重放末窗口 → 持久化足迹约 V4-Flash 的 **1/8** |

异构 KV 关系（main / indexer / SWA）见 [SWA 答疑 §1](../versions/qa/v4-swa-sliding-window.md) 与专图。

**与 §3.7 / §3.8 的关系**：算法上接续 CSA→CSA2 与异构 KV；infra 上改写持久化策略；多模态从 Visual Primitives 支线进入 **预训练起即原生** 的 Flash 旗舰；agent 评测常挂 [DeepSeek Harness](../versions/ds-harness.md) scaffold。
...
| [DeepSeek-V4.1-Flash](../versions/v4-1-flash.md) | V4 之后的 **CED / CSA2 / FP4 KV / SWA Bounded Replay**；接续 §3.7b |
