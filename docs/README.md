...
我们从 DeepSeek **最早公开的技术报告**一路跟到 **V4**，并把**大部分**（不是每一篇都全覆盖）主要技术文章里的**机制与细节**拆开写清楚：架构怎么变、训练/推理在优化什么、版本之间如何衔接。
...
本仓库的一大特性是**连贯、友好的阅读体验**：从 [中文导读](README.md) 或 [演进总览](reports/deepseek-version-lineage-20260625.md) 进入任意梗概、逻辑详解或 `qa/` 答疑后，文首均有**反向引用**——可一键回到来源章节、专题索引、[仓库首页（EN）](../README.md)。深入 DSA / Engram / MTP 等子专题后，同样能沿链接原路返回，阅读路径不断档。
