...
> [← DSA 梗概](../versions/dsa-sparse-attention.md) · [DSA 逻辑](./dsa-logic.md) · [§1 在 DSA 中的角色](#1-在-dsa-中的角色) · [§1 答疑：常数极小](#答疑为何-oleftl2right-却说常数极小) · [Decode 一步 walkthrough](#decode-forward-walkthrough)  
...
因此「$O(L^2)$」主要是 **Prefill / 训练**口径；Decode 每步是线性的，但**每层仍要扫全长 Indexer-Cache**。表里与 [dsa-logic](./dsa-logic.md) 沿用同一说法：**相对全长历史做 routing，阶与稠密 QK 相同，常数不同**。
...
[直接打开 SVG](./diagrams/lightning-indexer-forward.svg)
...
[直接打开 SVG](./diagrams/ess-dual-cache.svg)
...
**实现要点**（[Raschka 全文 §4](../reports/raschka-technical-deepseek-v3-v32.md#4-deepseek-v32-exp-and-sparse-attention)）：
...
DSA 把 KV 拆成两类（[dsa-logic.md §4](./dsa-logic.md#4-异构-kv-cacheindexer-cache-与-latent-cache)）：
...
3. 本地图：见 [§2 walkthrough 图](#decode-forward-walkthrough) · [§2 ess-dual-cache 图](#decode-一步分工) · [dsa-pipeline（DSA 逻辑）](./dsa-logic.md)
