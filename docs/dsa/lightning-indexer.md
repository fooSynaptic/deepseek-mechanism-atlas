...
> [← 中文导读](../README.md) · [← 仓库首页（EN）](../../README.md) · [← DSA 系列导读](./README.md) · [← 演进总览 §3.6](../reports/deepseek-version-lineage-20260625.md#36-deepseek-v32--v32-exp)

> [← DSA 梗概](../versions/dsa-sparse-attention.md) · [DSA 逻辑](./dsa-logic.md) · [§1 在 DSA 中的角色](#1-在-dsa-中的角色) · [§1 答疑：O(L²) 与常数极小](#答疑为何-oleftl2right-却说常数极小) · [为何不用 softmax](#为何-indexer-不用-softmax) · [Decode 一步 walkthrough](#decode-forward-walkthrough)
> **论文**：[DeepSeek-V3.2 arXiv:2512.02556](https://arxiv.org/pdf/2512.02556) · **第三方**：[Raschka §4](../reports/raschka-technical-deepseek-v3-v32.md#4-deepseek-v32-exp-and-sparse-attention)

---

## 1. 在 DSA 中的角色

**Lightning Indexer** 是 [DeepSeek Sparse Attention (DSA)](../versions/dsa-sparse-attention.md) 的**第一阶段**：在 **Core MLA** 做昂贵、精度敏感的主 attention 之前，用**极低成本**决定「当前 token 该 attend 哪些历史位置」，供 **Top-$k$ Selector** 选出 $k{=}2048$ 个历史下标。

每层 DSA 流水线：**Indexer 选位置** → Top-$k$ 得 index 集 $I$ → **Core MLA 只对 $I$ 内 entry 算 attention**。[Indexer-Cache 与 Latent-Cache 分工](#4-indexer-cache存什么为何不-offload) · [ess-dual-cache 图](#decode-一步分工)。
...
| 是否 softmax | **否**（[为何？](#为何-indexer-不用-softmax)） | **是**（精度主路径） |
| 是否读 512 维 $c^{KV}$ | **否**（读 indexer 专用 cache） | **是**（升维后做 MLA） |

<a id="答疑为何-oleftl2right-却说常数极小"></a>
...
因此「$O(L^2)$」主要是 **Prefill / 训练**口径；Decode 每步是线性的，但**每层仍要扫全长 Indexer-Cache**。表里与 [DSA逻辑详解](./dsa-logic.md) 沿用同一说法：**相对全长历史做 routing，阶与稠密 QK 相同，常数不同**。
...
| 是否在全长 $L$ 上归一化 | **否** | DSA 下在 **$k$ 条子集**上 softmax |

**训练 vs 推理**：推理时 Indexer **故意省略** softmax；**训练**阶段仍可用 **KL 散度** 等目标，让 Indexer 的选中模式 **对齐** 稠密 MLA 的 attention 分布（见 [§7 训练](#7-训练如何不损精度)）。学会「看谁」之后，推理用 **廉价打分 + hard Top-$k$** 即可；**精度主路径的 softmax 留给 Core MLA**，且 **只对 $k{=}2048$ 条** latent 做，不再扫全长。

> **小结**：Indexer = **全长粗筛（cheap score）**；Core MLA = **子集精算（softmax + $V$）**。ReLU 提供非线性，**真正稀疏来自 Top-$k$**（见 [§4 打分公式](#4-打分公式)），不是 ReLU 把大部分分数截成 0。
...
- **Prefill**：层数 $\times$ 全长 $\times$ 每层独立 indexer → 长上下文下 indexer 可占 **Prefill 大部分时间**（IndexCache 论文引 V3.2 场景约 **~81%**；[§8 推理与 infra](#8-推理与-infra)）。
- **Decode**：每步 $O(L)$ 且 **每层都跑**；$L{=}128\text{K}$ 时累加仍可观。
- **DSA 总收益**：Indexer 贵但 **只 routing**；Core MLA 从 $O(L^2)$ 收到 **$O(Lk)$**（$k{=}2048$），端到端长上下文仍明显变快。

[Index Share](./index-share-logic.md) 动的是 **重复 indexer 算子**（层间复用 Top-$k$），不是否认「单算 indexer 很轻」，而是 **F 层算一次、S 层跳过**。
...
上面是概念层：**固定当前 $t$，遍历历史 $s$ 打分，Top-$k$ 选下标**。下面用 **$L=8$、当前 $t=9$** 的一次 decode 前向（实际 $L$ 可达 128K），把**输入 → 前向 → 输出**三块与实现对齐。

<img src="./diagrams/lightning-indexer-forward.svg" alt="Decode 一步 Indexer 前向：固定 t=9，遍历 s=1..L 得 I_{t,s}，Top-k 得 I；输入 h_t 与 Indexer-Cache，输出分数向量与 index 集" width="920"/>

[图示详情](./diagrams/lightning-indexer-forward.svg)
...
<img src="./diagrams/ess-dual-cache.svg" alt="MLA Decode 一步: Indexer 选 top-2048 位置 vs Latent-Cache 升维并做 Core MLA" width="920"/>

[图示详情](./diagrams/ess-dual-cache.svg)
...
<img src="./diagrams/lightning-indexer-t-s-direction.svg" alt="位置 t/s 与 Top-k 方向：当前 q_t 对历史 Indexer-Cache 各行 k_s 打分，Top-k 选出 k 个 s；j 为多头编号" width="920"/>

[图示详情](./diagrams/lightning-indexer-t-s-direction.svg)

图中历史下标有时写作 $j$（如 $\mathrm{indexer}_j$），与公式里的 **$s$ 同义**；不要与 $q_{t,j}$ 里的头编号 $j$ 混用。

### Top-$k$：谁对谁选？

**一句话**：**当前 step 的 query，在全部历史 token 里选出 $k$ 个最相关的下标**（与 [§2 walkthrough](#decode-forward-walkthrough) 一致）。

Decode 每推一个新 token，$t$ 前进一格，**重新**对更新后的全长历史做一遍 Top-$k$（[Index Share](../versions/index-share.md) 可在层间复用 $I$，那是 infra 优化，不改变「$q_t$ 选历史 $s$」的方向）。
...
DSA 把 KV 拆成两类（[DSA逻辑详解§4](./dsa-logic.md#4-异构-kv-cacheindexer-cache-与-latent-cache)）：
...
| GPU 常驻 | **是** | 可 [ESS offload](../versions/ess-latent-cache-offload.md) |
...
DSA 的 $k$ 个位置是 **数据驱动、内容相关** 的，不是固定局部窗口（[Raschka 表 4-1](../reports/raschka-technical-deepseek-v3-v32.md#表-4-1滑动窗口注意力-vs-dsa)）。
...
- 结果：公开 benchmark 与 V3.1-Terminus **基本持平**，长上下文 **算力显著下降**（[V3.2 梗概](../versions/v3-2.md)）。
...
| **[Index Share](../versions/index-share.md)** | 跨层 **复用 top-$k$ index**，减 **重复跑 indexer 算子** |
| **Prefill 瓶颈** | 长上下文下 indexer 可占 Prefill **~81%**（IndexCache 论文引 V3.2 场景） |

Indexer **不做**：[softmax](#decode-一步分工)、[latent 升维](#decode-一步分工)、读 [$c^{KV}$](#decode-一步分工)、[offload](#5-indexer-cache存什么为何不-offload)。
...
| 方向 | 文档 |
|------|------|
| 前置 | [MLA](../versions/mla-latent-attention.md) · [V3.1 Hybrid](../versions/v3-1.md) |
| 同流水线 | [Top-$k$ + Core MLA](./dsa-logic.md#32-top-k-selector) |
| 下游 infra | [Index Share](./index-share-logic.md) · [ESS](../versions/ess-latent-cache-offload.md) |
| 下游算法 | [CSA/HCA 专文 §2](../versions/csa-hca-mixed-attention.md#csa-compressed-sparse) · [V4 梗概](../versions/v4.md)（新一代压缩 + 稀疏） |
...
2. Sebastian Raschka. *From DeepSeek V3 to V3.2* — [§4 Sparse Attention](../reports/raschka-technical-deepseek-v3-v32.md#4-deepseek-v32-exp-and-sparse-attention) · [要点速读](../reports/raschka-technical-deepseek-v3-v32-highlights.md#与本地文档映射)
3. 本地图：[§2 walkthrough 图](#decode-forward-walkthrough) · [ess-dual-cache 图](#decode-一步分工) · [dsa-pipeline](./dsa-logic.md)
