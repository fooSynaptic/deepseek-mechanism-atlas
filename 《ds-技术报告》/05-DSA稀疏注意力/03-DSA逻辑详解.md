...
> [← 中文导读](../00-前言/02-中文导读.md) · [← 仓库首页（EN）](../00-前言/02-中文导读.md) · [← 系列目录](01-系列导读.md) · **[DSA 梗概](02-DSA梗概.md)** · [Index Share 逻辑](06-Index-Share逻辑.md) · [V3.2 梗概](../04-版本代际/02-V3.2-DSA.md) · [演进总览 §3.6](../01-总览/01-版本演进总览.md#36-deepseek-v32--v32-exp) · [Raschka §4](../08-外部解读/01-Raschka要点速读.md#与本地文档映射)
...
> **梗概**：[docs/versions/v3-2.md](../04-版本代际/02-V3.2-DSA.md)
...
<img src="figures/dsa-pipeline.svg" alt="DSA 两阶段：Lightning Indexer → Top-k → Core MLA；Indexer-Cache 与 Latent-Cache" width="920"/>

[直接打开 SVG](figures/dsa-pipeline.svg)

### 3.1 Lightning Indexer（廉价全扫）

> **专题**：[lightning-indexer.md](04-Lightning-Indexer详解.md)（打分公式 $I_{t,s}$、Indexer-Cache、训练对齐、DeepGEMM）

- 用 **极低 head 维** 的点积：**当前** indexer query $q_t$ 对 **每条历史** token $s$ 的 indexer key $k_s$（Indexer-Cache 第 $s$ 行）打分，得 $I_{t,s}$（见 [lightning-indexer walkthrough](04-Lightning-Indexer详解.md#decode-forward-walkthrough)）。
...
| **Latent-Cache** | 核心 MLA attention 的 latent KV | ~83.2% | 仅 **被选中的 top-$k$ entry** 进入主 attention | 可 offload（[ESS](../06-推理基础设施/01-ESS概念.md)） |

**设计含义**：

1. **Indexer-Cache 必须留在 GPU**：每 decode step 都要对全长跑 indexer，offload 会拖垮延迟。
2. **Latent-Cache 可分层**：主 attention 只 touch $k$ 个 entry → [ESS](../06-推理基础设施/01-ESS概念.md) 等方案可把 **低频、未选中** 的 latent 放到 CPU，按稀疏访问拉回。
...
> **延伸**：[Index Share 逻辑](06-Index-Share逻辑.md) · [ESS 详解](../06-推理基础设施/01-ESS概念.md)

---

## 5. 与 V3.1 Hybrid 推理的关系

V3.1 引入 **Hybrid**：Prefill 用 MHA 形态、Decode 用 MQA 形态，降低 decode KV 带宽（MHA / GQA / MQA 对照见 [V3.1 梗概 §MLA 模式切换](../04-版本代际/01-V3.1-Terminus.md#mla-模式切换terminus-起)）。DSA 在 V3.2 上 **叠加稀疏选择**，不是推翻 MLA：
...
| **[ESS](../06-推理基础设施/01-ESS概念.md)** | Latent-Cache CPU offload，与 Index Share **正交可叠加** |
...
| 方向 | 关系 |
|------|------|
| 上游 | [V3.1-Terminus](../04-版本代际/01-V3.1-Terminus.md)：128K + Hybrid，无 DSA |
| 同架构 | V3.2-Exp → V3.2 正式版 |
| 下游 infra | [Index Share](06-Index-Share逻辑.md)、[ESS](../06-推理基础设施/01-ESS概念.md) |
| 下游算法 | [CSA/HCA 混合压缩注意力](../04-版本代际/05-CSA-HCA混合压缩注意力.md) · [V4 梗概](../04-版本代际/03-V4.md)：压缩 + 稀疏的下一代 |
| 正交 | [Engram 条件记忆](../07-Engram/02-Engram系列导读.md)：n-gram 查表稀疏轴 |
...
- 一页梗概：[v3-2.md](../04-版本代际/02-V3.2-DSA.md)
- 系列目录：[./README.md](01-系列导读.md)
...
| ← 上一章 | 下一章 → |
|----------|----------|
| [DSA（DeepSeek Sparse Attention）稀疏注意力](02-DSA梗概.md) | [Lightning Indexer 详解](04-Lightning-Indexer详解.md) |
