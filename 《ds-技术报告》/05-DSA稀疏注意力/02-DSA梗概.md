...
[直接打开 SVG](figures/dsa-pipeline.svg) · [系列目录](01-系列导读.md)
...
**与滑动窗口的区别**：DSA 的 $k$ 个位置是 **学到的、内容相关** 的，不是固定局部窗口（见 [Raschka §4](../08-外部解读/01-Raschka要点速读.md)）。

> **Lightning Indexer 专题**：[lightning-indexer.md](04-Lightning-Indexer详解.md)（打分公式、Indexer-Cache、训练与 infra；含论文与 Raschka 引用）
...
| **上游 ① 同质 MLA KV** | [mla-latent-attention.md](../02-基座架构/02-MLA低秩注意力.md) |
| **下游 ③ Index Share** | [index-share.md](05-Index-Share梗概.md)（indexer 算力，**并列**） |
| **下游 ④ ESS** | [ess-latent-cache-offload.md](../06-推理基础设施/01-ESS概念.md)（Latent offload，**并列**） |
| **下游 ⑤ V4 infra** | [v4.md §推理 infra](../04-版本代际/03-V4.md#推理-infra-关注点) · [KV layout](../06-推理基础设施/05-V4-KV-Layout.md) · [HiSparse](../06-推理基础设施/06-V4-HiSparse.md) · [磁盘 prefix](../06-推理基础设施/07-V4-磁盘Prefix-Cache.md) |
...
| **上游 ① MLA** | [mla-latent-attention.md](../02-基座架构/02-MLA低秩注意力.md) |
| **下游 ③ CSA/HCA** | [csa-hca-mixed-attention.md](../04-版本代际/05-CSA-HCA混合压缩注意力.md) · [v4.md](../04-版本代际/03-V4.md) |
...
| **Index Share** | **不改 DSA 算法**；跨层复用 top-$k$ index，减 indexer 重复计算（[index-share.md](05-Index-Share梗概.md)） |
...
- **MLA**：K/V 压入 latent 再缓存（见 [mla-latent-attention.md](../02-基座架构/02-MLA低秩注意力.md)）
- **DSA**：在 MLA latent **序列**上增加「选哪些位置参与 attention」
- V3.1 **Hybrid**（Prefill MHA / Decode MQA）仍是 DSA 的前置（[v3-1.md](../04-版本代际/01-V3.1-Terminus.md)）
...
| 资源 | 说明 |
|------|------|
| [../dsa/lightning-indexer.md](04-Lightning-Indexer详解.md) | **Lightning Indexer** 公式、Indexer-Cache、与滑动窗对比 |
| [../dsa/dsa-logic.md](03-DSA逻辑详解.md) | 完整机制、与 Hybrid/ESS/Engram 关系 |
| [../dsa/index-share-logic.md](06-Index-Share逻辑.md) | Index Share `FFFS` 模式 |
| [v3-2.md](../04-版本代际/02-V3.2-DSA.md) | V3.2 版本梗概 |
...
| ← 上一章 | 下一章 → |
|----------|----------|
| [DeepSeek DSA 与 Index Share 系列](01-系列导读.md) | [DSA（DeepSeek Sparse Attention）逻辑详解](03-DSA逻辑详解.md) |
