...
因此「$O(L^2)$」主要是 **Prefill / 训练**口径；Decode 每步是线性的，但**每层仍要扫全长 Indexer-Cache**。表里与 [DSA逻辑详解](03-DSA逻辑详解.md) 沿用同一说法：**相对全长历史做 routing，阶与稠密 QK 相同，常数不同**。
...
[图示详情](figures/lightning-indexer-forward.svg)
...
[图示详情](figures/ess-dual-cache.svg)
...
<img src="figures/lightning-indexer-t-s-direction.svg" alt="位置 t/s 与 Top-k 方向：当前 q_t 对历史 Indexer-Cache 各行 k_s 打分，Top-k 选出 k 个 s；j 为多头编号" width="920"/>

[图示详情](figures/lightning-indexer-t-s-direction.svg)
...
DSA 把 KV 拆成两类（[DSA逻辑详解§4](03-DSA逻辑详解.md#4-异构-kv-cacheindexer-cache-与-latent-cache)）：
...
3. 本地图：[§2 walkthrough 图](#decode-forward-walkthrough) · [ess-dual-cache 图](#decode-一步分工) · [dsa-pipeline](03-DSA逻辑详解.md)