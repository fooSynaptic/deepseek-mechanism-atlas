...
[← 返回 Hash MoE §1.2](../../04-版本代际/06-Hash-MoE-FP4.md#hash-moe-routing) · [EP 答疑 §4](moe-expert-parallel-ep.md#4-hash-moe-改什么不改什么) · [centroid vs gate-weight §4.4](moe-centroid-vs-gate-weight.md#44-v4-hash-moe部分层离开-centroid-路由) · [答疑目录](README.md)
...
| 负载均衡 | hash 设计 **静态近似均匀** | 动态 bias $b_i$ + [$L_{\mathrm{Bal}}$](../04-序列均衡损失.md) |
...
V4 **继承** DeepSeekMoE **256/8 + shared** 与 **sigmoid + bias** 路由（[aux-loss-free](../03-aux-loss-free-MoE路由.md)）。深层不动 → **大部分 MoE 层** 权重形态、训推 kernel、EP 路径与 V3 **同族**，只在前几层插入 Hash 块；若 **全栈 Hash**，router 权重与 checkpoint 语义 **全面重写**，风险与验证成本都高。
...
- **FP4** 作用在 **routed expert 权重** 上，与 **Hash vs centroid** 正交（[Hash MoE 专文 §2](../../04-版本代际/06-Hash-MoE-FP4.md#fp4-moe-quant)），深层 routed 也可 FP4，**router 逻辑仍 centroid**。
...
- [Hash MoE + FP4 专文](../../04-版本代际/06-Hash-MoE-FP4.md)
- [aux-loss-free 路由](../03-aux-loss-free-MoE路由.md)
