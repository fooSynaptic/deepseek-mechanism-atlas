...
[← 返回 DeepSeekMoE §前向公式](../05-DeepSeekMoE.md#forward-formula) · [答疑目录](README.md)
...
DeepSeekMoE 把 $N$ 个大 expert 拆成 $2N$ 个小 expert（见 [Figure 2 优化逻辑](../05-DeepSeekMoE.md#optimization-logic)）：
...
### 3.3 与 device-limited routing 一致（V2 §2.2.2）
...
### 4.2 V3 aux-loss-free：**拆选择 vs 门控**（关键延伸）
...
详见 [aux-loss-free 路由逻辑](../03-aux-loss-free-MoE路由.md)。
...
[$L_{\mathrm{Bal}}$](../04-序列均衡损失.md) 在 **单序列内** 补 $f_i P_i$，与 $b_i$ 的 batch 级均衡互补 — 仍假设 expert 是可被 **按亲和度分配** 的特化单元，而不是纯随机 gate。

<a id="44-v4-hash-moe部分层离开-centroid-路由"></a>

### 4.4 V4 Hash MoE：部分层离开 centroid 路由

V4 前几层改用 **Hash 路由**（见 [hash-moe-fp4.md](../../04-版本代际/06-Hash-MoE-FP4.md)）— 不再用 $u^\top e_i$ 做语义匹配，而是确定性 hash。
