...
[直接打开 SVG](figures/mla-forward-flow.svg)
...
> V3.1 **Hybrid**：Prefill 时 Q/K 可按 **MHA 式**展开 latent；Decode 时 **MQA 式**共享 latent（见 [v3-1.md §MLA 模式切换](../04-版本代际/01-V3.1-Terminus.md#mla-模式切换terminus-起)）。**cache 布局仍是 MLA latent**，切换的是展开方式。
...
| **下游 ② 异构 cache** | [dsa-sparse-attention.md §异构 KV](../05-DSA稀疏注意力/02-DSA梗概.md#异构-kv-cache) |
...
| **下游 ② DSA** | [dsa-sparse-attention.md](../05-DSA稀疏注意力/02-DSA梗概.md)（MLA 结构不变，加稀疏选择） |
| **下游 ③ CSA/HCA** | [csa-hca-mixed-attention.md](../04-版本代际/05-CSA-HCA混合压缩注意力.md) · [v4.md](../04-版本代际/03-V4.md)（新注意力，不再单一 MLA latent） |
...
| **V3.2 + DSA** | MLA **不变**；indexer 在 latent 序列上选 top-$k$，Core 仍做 MLA（见 [dsa-sparse-attention.md](../05-DSA稀疏注意力/02-DSA梗概.md)） |
| **V4** | CSA/HCA 等 **新注意力**，不再单一 MLA latent（见 [csa-hca-mixed-attention.md](../04-版本代际/05-CSA-HCA混合压缩注意力.md) · [v4.md](../04-版本代际/03-V4.md)） |
...
| 资源 | 说明 |
|------|------|
| [mla-详解.md（含 PyTorch 对照）](02-MLA低秩注意力.md) | 更长的 shape 推演与 `mla_forward.py` |
| [FlashMLA](https://github.com/deepseek-ai/FlashMLA) | V3+ 推理 kernel |
| [v3.md](01-V3基座.md) | V3 梗概中的 MLA 一行 |
...
| ← 上一章 | 下一章 → |
|----------|----------|
| [DeepSeek-V3 梗概](01-V3基座.md) | [DeepSeekMoE 架构](05-DeepSeekMoE.md) |
