...
> [← 中文导读](../00-前言/02-中文导读.md) · [← 仓库首页（EN）](../../README.md) · [← 演进总览 §3.3](../01-总览/01-版本演进总览.md#33-deepseek-v3) · [← MoE 线导读](../01-总览/07-MoE线导读.md) · [← DeepSeekMoE 上游](05-DeepSeekMoE.md) · [← V3 梗概](01-V3基座.md) · [版本目录](../01-总览/02-版本梗概索引.md)  
...
V3 起标配；V4 系仍沿用同一思路（论文称 *auxiliary-loss-free load balancing*，$\gamma \approx 0.001$）。
...
## aux-loss-free 核心逻辑（V3 论文 Eq. 16）
...
### 2. 每个训练 step 末尾更新 $b_i$（batch-wise）

监控 **整个 batch**（一步训练）上各 expert 的负载：

```
若 expert i 过载（overload）  →  b_i -= γ
若 expert i 欠载（underload） →  b_i += γ
```
...
专家 $i$ 在 token $t$ 被激活，当且仅当 $s_{i,t} + b_i$ 落在 top-$K_r$ 集合内；否则 $g'_{i,t}=0$。门控 $g_{i,t}$ 仍由 $s_{i,t}$ 经 top-$K$ 内归一化得到（论文 Eq. 16–19）。
...
| 专家 specialization | sequence 内被压平 | 允许 **不同领域** 走不同专家（论文 Fig. 9） |
| 超参 | aux loss 系数 $\alpha$ 难调 | $\gamma$（bias 步长） |
| 与主任务冲突 | 大 $\alpha$ 伤性能 | 论文 ablation：**多数 benchmark 更优** |

### 表：V3 论文 Table 5 摘要（aux-loss-free vs 纯 aux-loss 基线）

**Small MoE**（激活 2.4B / 总 15.7B / 1.33T tokens）
...
**Large MoE**（激活 20.9B / 总 228.7B / 578B tokens）
...
aux-loss-free **不是**完全不管极端倾斜。V3 还保留 **互补的 sequence-wise balance loss** $L_{\mathrm{Bal}}$（论文 Eq. 17–20），防止 **单条序列内** 专家极度失衡——但 **主均衡机制** 仍是 bias 方案。

> **详解**：[moe-sequence-wise-balance-loss.md](04-序列均衡损失.md) — $f_i$、$P_i$ 如何在每条序列上统计、$f_i P_i$ 如何反传拉平负载、与 $b_i$ 的分工。
...
DeepSeek-V3 细粒度 MoE + sigmoid router 在 Megatron 侧常配合 **FlexDispatcher + DeepEP** 做 token dispatch（见 [Megatron MoE README](https://github.com/NVIDIA/Megatron-LM/blob/main/megatron/core/transformer/moe/README.md)）。
...
| **上游 ② DeepSeekMoE** | [deepseek-moe.md](05-DeepSeekMoE.md) |
| **并列 ④ $L_{\mathrm{Bal}}$** | [moe-sequence-wise-balance-loss.md](04-序列均衡损失.md) |
| **下游 ⑤ Hash MoE** | [hash-moe-fp4.md](../04-版本代际/06-Hash-MoE-FP4.md) · [v4.md](../04-版本代际/03-V4.md) |
...
- 本地：[moe-sequence-wise-balance-loss.md](04-序列均衡损失.md) · [v3.md §DeepSeekMoE](01-V3基座.md) · [Raschka V3 导读](../08-外部解读/01-Raschka要点速读.md)
...
| ← 上一章 | 下一章 → |
|----------|----------|
| [DeepSeekMoE 架构](05-DeepSeekMoE.md) | [MoE sequence-wise balance loss（$L_{\mathrm{Bal}}$）](04-序列均衡损失.md) |
