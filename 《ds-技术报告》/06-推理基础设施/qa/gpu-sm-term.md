# 名词解释：SM（Streaming Multiprocessor）

[← 返回投机解码专文 §1.1](../04-DSpark投机解码.md#11-瓶颈每-token-搬一遍大模型权重) · [Compute vs Memory 答疑](spec-decode-compute-vs-memory-bound.md) · [MMA 名词](../../02-基座架构/qa/fp8-mma-term.md) · [答疑目录](../../01-总览/qa/README.md)
...
L2 Cache（片上，全芯片共享）
  ↓
SM × N（每颗 GPU 有几十到一百多个 SM，例如 H100 132 SM、H20 类似量级）
  ├─ 寄存器 / Shared Memory（最快，容量 KB 级，每 SM 私有）
  ├─ CUDA Core（通用 FP32/INT 等）
  └─ Tensor Core（专用 GEMM/MMA，吞吐高）
```
...
|------|----------------|----------------|
| **CUDA Core** | 通用标量/向量运算、控制流、部分 epilogue | FP8 反量化、partial sum **FP32 续累加**（[fp8-mma-term.md](../../02-基座架构/qa/fp8-mma-term.md)） |
...
[投机解码专文 §1.1](../04-DSpark投机解码.md#11-瓶颈每-token-搬一遍大模型权重) 与 [Compute vs Memory 答疑 §2](spec-decode-compute-vs-memory-bound.md#2-为何-decode-默认是-memory-bound) 的物理图景：
...
投机解码 target **verify**：仍 **搬 1 遍** 权重进 SM，但在 **$K$ 个位置并行算**——FLOPS 增加，但 **不再为 $K$ 个 token 各搬 $K$ 遍权重**（见 [spec-decode-compute-vs-memory-bound.md §2](spec-decode-compute-vs-memory-bound.md#2-为何-decode-默认是-memory-bound)）。
...
| **MMA** | **Tensor Core 指令** | Matrix Multiply-Accumulate（[fp8-mma-term.md](../../02-基座架构/qa/fp8-mma-term.md)） |
...
| 文档 | 锚点 / 说明 |
|------|-------------|
| [投机解码专文 §1.1](../04-DSpark投机解码.md#11-瓶颈每-token-搬一遍大模型权重) | HBM → 计算单元；GEMV Memory-Bound |
| [Compute vs Memory 答疑 §2–§4](spec-decode-compute-vs-memory-bound.md#2-为何-decode-默认是-memory-bound) | 「搬进 SM」「填不满 SM」 |
| [酱紫君解读 §半自回归](../../08-外部解读/03-酱紫君DSpark阅读笔记.md#dspark-半自回归草稿并行主干-vs-顺序头) | 顺序头 **寄存器 / on-chip** vs HBM 拉 MoE |
| [fp8-mma-term.md](../../02-基座架构/qa/fp8-mma-term.md) | SM 内 Tensor Core vs CUDA Core 分工 |
