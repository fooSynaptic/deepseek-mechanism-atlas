...
块压缩（4:1 / 128:1）会 **损失 token 级精度**。最近几十个 token 对语法、指代、工具调用格式等 **极敏感**；SWA 用 **滑动窗口** 保留这部分 **dense 局部**，与 CSA top-$k$、HCA dense 摘要 **并行参与** 同一层 attention 融合（见 [CSA/HCA 专文 §4](../../04-版本代际/05-CSA-HCA混合压缩注意力.md#v4-mixed-attention)）。
...
| 文档 | 内容 |
|------|------|
| [v4-kv-layout.md](../05-V4-KV-Layout.md) | Classical vs State 双池；SWA 进 State |
| [v4-disk-prefix-cache.md](../07-V4-磁盘Prefix-Cache.md) | SWA 三档落盘策略 |
| [v4-hisparse.md](../06-V4-HiSparse.md) | SWA 与 C4 offload 的容量权衡 |
