...
> **答疑**：[酱紫君解读：验证截断对效率与准确率的影响](../08-外部解读/03-酱紫君DSpark阅读笔记.md#置信度调度与验证截断)
...
| 项 | 说明 |
|----|------|
| **不是新基座** | 与 [V4](../04-版本代际/03-V4.md) 同 checkpoint；`DeepSeek-V4-*-DSpark` = target + 投机模块 |
| **不是 KV / offload** | 与 [ESS](01-ESS概念.md)、[Index Share](../05-DSA稀疏注意力/05-Index-Share梗概.md) **正交** |
...
| 材料 | 说明 |
|------|------|
| [酱紫君（GalAster）：DSpark 与投机解码](../08-外部解读/03-酱紫君DSpark阅读笔记.md) | 投机背景、半自回归 draft、验证截断、MTP 融合、draft 训练 vs fine-tune；[知乎原文](https://www.zhihu.com/question/2054255700407055156/answer/2054537823915619954) |
| [批量验证机制笔记](../../docs/wiki/投机解码之批量验证.md) | 目标模型单次前向并行验 K 个 draft token |
