...
## 为何需要 HiSparse（相对 ESS）

| 维度 | **ESS（V3.2）** | **HiSparse（V4）** |
|------|----------------|------------------|
| Cache 结构 | 同质 Latent-Cache（per-token MLA） | **CSA/HCA/SWA/Indexer/tail** 异构（见 [KV layout](05-V4-KV-Layout.md)） |
| Offload 粒度 | 按 **latent entry**；依赖 DSA top-$k$ **时间局部性** | 按 **C4 压缩 entry**；依赖 CSA **空间稀疏激活** |
| Indexer | GPU 常驻 | V4 indexer 仍参与 CSA 路径；layout 见 [v4.md](../04-版本代际/03-V4.md) |
...
| **并列 prefix** | [v4-disk-prefix-cache.md](07-V4-磁盘Prefix-Cache.md) |
| **上游 ④ ESS** | [ess-latent-cache-offload.md](01-ESS概念.md)（**非**简单放大） |
...
| ← 上一章 | 下一章 → |
|----------|----------|
| [V4 KV Layout：Classical + State 双池（论文 §3.5.1）](05-V4-KV-Layout.md) | [V4 磁盘 Prefix Cache（论文 §3.5.2）](07-V4-磁盘Prefix-Cache.md) |
