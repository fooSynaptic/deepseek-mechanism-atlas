...
```
DeepSeek-V3 Base
    ├─[纯 RL]────────────────────────────► DeepSeek-R1-Zero
    │                                         │ 采样/过滤/精炼
    └─[冷启动SFT→RL→拒绝采样SFT→RL]──────► DeepSeek-R1
```

---

## 支路 A：DeepSeek-R1-Zero（§2）
...
## 支路 B：DeepSeek-R1 四阶段（§3）

中间 checkpoint：**Dev-1 → Dev-2 → Dev-3 → R1**（论文 Figure 2 / Table 3）
...
```
Dev-2 / R1-Zero / V3 ──采样──► Filter ──► Refine(V3+Human) ──► SFT 混合数据
```
...
| [training-pipeline.md](training-pipeline.md) | 本文 |
| [diagrams/deepseek-r1-training-pipeline.svg](diagrams/deepseek-r1-training-pipeline.svg) | 自绘流程图 |
| [assets/figures/training-pipeline-reference.png](assets/figures/training-pipeline-reference.png) | 外部参考图 |
| [src/deepseek-r1-2501.12948.pdf](src/deepseek-r1-2501.12948.pdf) | 原文 |
