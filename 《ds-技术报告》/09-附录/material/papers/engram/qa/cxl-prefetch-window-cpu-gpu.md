...
Engram 索引 **[只依赖 input token](../diagrams/engram-01d-multi-head-hash.svg)**，因此：
...
这就是「**CPU 侧先算索引**」对 prefetch **不可或缺**——不是因为 CPU 算力比 GPU 强，而是因为 **[这件事本来就不该占 GPU，且可以极早做](#基础概念-decode-layer-hidden)**。
...
- [DeepSeek Engram 系列导读§缓存访问逻辑](../../../../../07-Engram/02-Engram系列导读.md#缓存访问逻辑一次-decode-的完整路径)
- [engram-02c-cxl-cache-access.svg](../diagrams/engram-02c-cxl-cache-access.svg)
- [L1 HBM / L2 DRAM / L3 CXL.mem：三级存储区别](cxl-l1-l2-l3-memory-tiers.md)
