...
> [← 中文导读](../../../../00-前言/02-中文导读.md) · [← Engram 官方 README](../../../../07-Engram/01-Engram官方README.md) · [← 演进总览 §7 Engram](../../../../01-总览/01-版本演进总览.md#7-与本仓库其他专题的关系) · [《ds-技术报告》](../../../../01-总览/01-版本演进总览.md#7-与本仓库其他专题的关系)  
> 更新：2026-06-24  
> PDF 目录：[src/](src)
...
>
> **Abstract**（[Engram 官方 README](../../../../07-Engram/01-Engram官方README.md)）：
>
...
<img src="diagrams/engram-00-series-lineage.svg" alt="Engram 四篇论文系列演进：奠基与 CXL / Nine / Tiny 三条线" width="920"/>

[直接打开 SVG](diagrams/engram-00-series-lineage.svg)
...
![Engram 双稀疏轴与 U 形分配律](diagrams/engram-01-sparsity-allocation.svg)
...
![Engram 架构：n-gram 查表 + 上下文门控 + 残差注入](assets/figures/engram-architecture-qbitai.png)

**在整网中的位置**（对应图左侧竖列）：

<img src="diagrams/engram-01b-backbone-placement.svg" alt="Engram 在 backbone 中的插入位置" width="520"/>

[直接打开 SVG](diagrams/engram-01b-backbone-placement.svg)
...
| 6 | 上下文门控 | **★ 过滤记忆** — $\alpha_t$ 门控，$\tilde{v}_t=\alpha_t v_t$（[为何记忆依赖？](qa/step6-context-gating-rationale.md)） |
| 7 | 短卷积 | **已过滤** — 只在 $\tilde{v}_t$ 上运算 |
| 8 | 残差写回 | **已过滤** — $Y$ 注入 backbone，之后接 Attn/MoE |

<img src="diagrams/engram-01c-forward-dataflow.svg" alt="Engram 逐步前向与记忆过滤标注" width="1170"/>

[直接打开逐步前向 SVG](diagrams/engram-01c-forward-dataflow.svg)
...
<img src="diagrams/engram-01d-multi-head-hash.svg" alt="Step 3 多头哈希寻址：K 个 hash 头映射到素数表并查 embedding" width="936"/>

[直接打开 SVG](diagrams/engram-01d-multi-head-hash.svg)
...
> **答疑**：[门控依据与记忆依赖过滤](qa/step6-context-gating-rationale.md)
...
- **感受野常数**（demo 默认 $w=4,N=3$）：门控流沿序列从 **1** 扩到 **$1+(w-1)N=10$**，不是扩到 4。详见 [感受野常数](qa/step7-short-conv-receptive-field.md)；**训练/推理差异**见 [RF 1→10 影响](qa/step7-rf10-train-infer-impact.md)。

<img src="diagrams/engram-01e-residual-step78.svg" alt="Step 7-8：ShortConv 残差路径与 backbone 写回" width="860"/>
...
见上文 **Step 1–8 总表** 与 [逐步前向 SVG](diagrams/engram-01c-forward-dataflow.svg)（每步顶部标注记忆过滤状态）。
...
![CXL 多机 Engram 内存池部署](diagrams/engram-02-cxl-pooling.svg)
...
> **答疑**：[HBM / DRAM / CXL.mem 区别与 L1–L3 分工](qa/cxl-l1-l2-l3-memory-tiers.md)
...
<img src="diagrams/engram-02b-cxl-hardware-topology.svg" alt="CXL Engram 三级存储硬件拓扑" width="920"/>

[直接打开 SVG](diagrams/engram-02b-cxl-hardware-topology.svg)
...
<img src="diagrams/engram-02c-cxl-cache-access.svg" alt="CXL Engram 缓存访问与 prefetch 时序" width="920"/>

[直接打开 SVG](diagrams/engram-02c-cxl-cache-access.svg)

> **答疑**：[prefetch window 与 CPU/GPU 分工](qa/cxl-prefetch-window-cpu-gpu.md) · [CXL vs RDMA pattern](qa/cxl-vs-rdma-communication-pattern.md)
...
> **答疑**：[CXL vs RDMA 通信 pattern（02c 图标注）](qa/cxl-vs-rdma-communication-pattern.md)
...
<img src="diagrams/engram-03-hot-cold-tier.svg" alt="热/冷双层路由、loss flip、碰撞正则（导读摘要图）" width="920"/>

[直接打开 SVG](diagrams/engram-03-hot-cold-tier.svg)
...
<img src="assets/figures/nine-2601.16531/fig1-two-tier-architecture.png" alt="Engram-Nine 双层检索架构（论文 Fig.1）" width="920"/>

[论文原图 PDF](src/hot-tier-2601.16531.pdf) · [裁剪图源目录](assets/figures/nine-2601.16531)
...
<img src="assets/figures/nine-2601.16531/table3-val-loss-summary.png" alt="各配置 val loss 与 hot_cold_delta 汇总（论文 Table 3）" width="920"/>
...
<img src="assets/figures/nine-2601.16531/fig2-hot-cold-flip.png" alt="Hot/Cold flip 训练动力学（论文 Fig.2）" width="920"/>

<img src="assets/figures/nine-2601.16531/table4-flip-timing.png" alt="Flip 时刻与前后最大优势（论文 Table 4）" width="920"/>
...
<img src="assets/figures/nine-2601.16531/table5-nhot-ablation.png" alt="N_hot 消融：flip 时刻与稀疏度假设证伪（论文 Table 5）" width="920"/>
...
<img src="assets/figures/nine-2601.16531/fig3-gate-evolution.png" alt="门控权重 α 随训练演化（论文 Fig.3）" width="920"/>

<img src="assets/figures/nine-2601.16531/table6-alpha-bucket.png" alt="按 α 分桶的 loss 与 hot 占比（论文 Table 6）" width="920"/>

<img src="assets/figures/nine-2601.16531/fig4-gate-mismatch.png" alt="门控错配：高 α 对应高 loss（论文 Fig.4）" width="920"/>

**补充 — Figure 5**：深层（Layer 6）$\Delta\alpha$ 可出现反转，Nine 配置更明显。

<img src="assets/figures/nine-2601.16531/fig5-layer-alpha-reversal.png" alt="各层门控偏好 Δα（论文 Fig.5）" width="720"/>
...
![触发词查表注入视觉概念](diagrams/engram-04-tiny-engram-vision.svg)
...
- [Engram 官方 README](../../../../07-Engram/01-Engram官方README.md)
