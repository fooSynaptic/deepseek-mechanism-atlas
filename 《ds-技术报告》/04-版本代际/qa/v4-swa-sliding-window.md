...
[← 返回 CSA/HCA 一句话](../05-CSA-HCA混合压缩注意力.md#一句话) · [§4 异构 KV](../05-CSA-HCA混合压缩注意力.md#v4-mixed-attention) · [KV Layout §State](../../06-推理基础设施/05-V4-KV-Layout.md#state-cache) · [磁盘 Prefix §SWA 三档](../../06-推理基础设施/07-V4-磁盘Prefix-Cache.md#swa-三档策略论文-352) · [答疑目录](../../01-总览/qa/README.md)
...
| **DeepSeek 落点** | V1–V3 主线走 **MLA → DSA → CSA/HCA**；**V4** 把 SWA 作为 **五类 KV 之一** 与压缩 entry **分池**（[KV Layout §State](../../06-推理基础设施/05-V4-KV-Layout.md#state-cache)），保证最近邻域 **不丢 token 级精度** |
...
![单层 Core Attention：main Q 分两路读 Top-K main KV 与 SWA KV，indexer 只负责选块](../figures/kv-types-core-attention.svg)
...
块压缩（4:1 / 128:1，以及 V4.1 的 CSA2 main KV）会 **损失 token 级精度**。最近几十个 token 对语法、指代、工具调用格式等 **极敏感**；SWA 用 **滑动窗口** 保留这部分 **dense 局部**，与 CSA / CSA2 的 top-$k$ 主路径、HCA dense 摘要 **[并行参与](../05-CSA-HCA混合压缩注意力.md#v4-mixed-attention)** 同一层 attention 融合。
...
| 现象 | 说明 |
|------|------|
| **体积** | Together 早期：全量 SWA 时 per-token KV 可 **高于** 仅 MLA 路径；瓶颈常在 **SWA state** 而非 CSA/HCA 本体（[HiSparse 专文](../../06-推理基础设施/06-V4-HiSparse.md)） |
| **Eviction** | 与 C4 inactive offload **正交**；可只保留 **高复用** SWA 检查点 |
| **磁盘 Prefix** | 论文 §3.5.2：**Full / Periodic Checkpointing / Zero** 三档——在 **存储带宽 vs 命中重算** 间取舍（[专文](../../06-推理基础设施/07-V4-磁盘Prefix-Cache.md#swa-三档策略论文-352)） |
...
| 文档 | 内容 |
|------|------|
| [V4 KV Layout](../../06-推理基础设施/05-V4-KV-Layout.md) | Classical vs State 双池；SWA 进 State |
| [V4 磁盘 Prefix Cache](../../06-推理基础设施/07-V4-磁盘Prefix-Cache.md) | SWA 三档落盘策略 |
| [V4 HiSparse](../../06-推理基础设施/06-V4-HiSparse.md) | SWA 与 C4 offload 的容量权衡 |
