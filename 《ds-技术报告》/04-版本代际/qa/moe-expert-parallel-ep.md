...
[← 返回 Hash MoE §1.3](../06-Hash-MoE-FP4.md#still-inherited) · [DeepSeekMoE §推理 infra](../../02-基座架构/05-DeepSeekMoE.md#推理-infra-关注点) · [答疑目录](../../01-总览/qa/README.md)
...
**Shared experts** 不走稀疏 scatter：每层 **恒激活**，与 routed 输出 **相加**（见 [DeepSeekMoE](../../02-基座架构/05-DeepSeekMoE.md)）。
...
因此 V3 的 [aux-loss-free](../../02-基座架构/03-aux-loss-free-MoE路由.md) / [$L_{\mathrm{Bal}}$](../../02-基座架构/04-序列均衡损失.md) 首要目标之一是 **batch 级 expert 均衡**——直接服务 **EP 训练与推理吞吐**。
...
> **读法**：[Hash MoE 专文](../06-Hash-MoE-FP4.md) 换的是 **浅层选 expert 的函数**；**一旦 id 确定**，后面仍是 DeepSeekMoE 的 **EP + shared** 引擎路径，与 V3 推理栈同族。
...
[Hash MoE + FP4](../06-Hash-MoE-FP4.md#fp4-moe-quant) 在 EP 路径上多一层约束：

- **Routed expert 权重** FP4 存 HBM；各 rank 的 expert 子集 **4bit 读 + 高精度累加**（引擎需 **分 kernel**）。
- **Shared** 路径通常仍用更高精度（与 [V3 FP8](../../02-基座架构/06-V3-FP8动态量化.md) 栈衔接）。
...
V3 训练文档常并列 **DualPipe、DeepEP、FP8**（见 [v3.md §排除项](../../02-基座架构/01-V3基座.md#v3-structure-excluded)）：**DeepEP** 是 **MoE token dispatch 的通信库**，属于 **训推系统层**，不是 Transformer 权重结构。
...
| **device-limited routing** | V2 限制 token 只打本地 expert，减 EP 通信（见 [fine-grained 答疑](../../02-基座架构/qa/moe-fine-grained-segmentation.md)） |

---

## 参考

- [DeepSeekMoE](../../02-基座架构/05-DeepSeekMoE.md) · [aux-loss-free](../../02-基座架构/03-aux-loss-free-MoE路由.md)
- [Hash MoE + FP4](../06-Hash-MoE-FP4.md)
