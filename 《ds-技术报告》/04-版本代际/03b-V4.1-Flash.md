...
> [← 中文导读](../00-前言/02-中文导读.md) · [← 仓库首页（EN）](https://github.com/fooSynaptic/deepseek-mechanism-atlas) · [← 版本目录](../01-总览/02-版本梗概索引.md) · [← 演进总览 §3.7b](../01-总览/01-版本演进总览.md#37b-deepseek-v41-flash) · [← DeepSeek-V4](03-V4.md) · [← CSA / HCA](05-CSA-HCA混合压缩注意力.md) · [← DSpark](../06-推理基础设施/04-DSpark投机解码.md) · [← Engram](../07-Engram/01-Engram官方README.md) · [← DeepSeek Harness](../10-Agent运行时/01-DeepSeek-Harness.md) · [← mHC](04-mHC流形约束超连接.md) · [← SWA 答疑](qa/v4-swa-sliding-window.md)
...
- **KV 极限压缩**：**CSA2**（跨层 KV / index 复用 + Hierarchical Sparse Indexer）+ **FP4 main KV** → 全局 KV（常驻 HBM）约 **890 B/token**，约为 [V4-Flash](03-V4.md) 的 **1/4**；**SWA Bounded Replay** 使持久化 KV（SSD / host）约为 V4-Flash 的 **1/8**。
- **V4.1-Flash的优化**：属于 [V4](03-V4.md) 之后的 **架构 + infer 联合升级**；本次模型升级新增特性 [CSA](05-CSA-HCA混合压缩注意力.md) 演进、[DSpark](../06-推理基础设施/04-DSpark投机解码.md)、[Engram](../07-Engram/01-Engram官方README.md)、[mHC，其中engram为首次官方开源实现，弥补了大模型领域开源社区的一块重要拼图](04-mHC流形约束超连接.md)；agent 评测中也涉及到 [DeepSeek Harness](../10-Agent运行时/01-DeepSeek-Harness.md) Minimal 等 scaffold。
...
在 [V4 CSA](05-CSA-HCA混合压缩注意力.md) 思路上，把压缩做到 **entry 大小 × 序列压缩 × 层间复用** 三个可乘维度。
...
| 组件                  | 要点                                                                             | 仓库专文                                                                                      |
| ------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| **Single-Pass mHC** | 修订残差流混合，便于 Mega-mHC kernel 融合                                                  | [mHC](04-mHC流形约束超连接.md)                                                |
| **Engram**          | 196B 条件记忆，按 token 稀疏 lookup                                                    | [Engram](../07-Engram/01-Engram官方README.md)                                                             |
| **DSpark**          | 骨干预训练 **省略 MTP**；投机解码用 DSpark，**骨干训完后单独训**                                     | [DSpark](../06-推理基础设施/04-DSpark投机解码.md)                                                |
| **MoE**             | 1 shared + **384** routed / 层，每 token 激活 **6** routed；图文分模态 aux-loss-free bias | [DeepSeekMoE](../02-基座架构/05-DeepSeekMoE.md) · [aux-loss-free](../02-基座架构/03-aux-loss-free-MoE路由.md)        |
| **Vision**          | DeepSeek-ViT（2D-RoPE、3×3 pixel-unshuffle → 视觉 token ÷9）+ 两层 MLP                | [Visual Primitives](../09-附录/03-Visual-Primitives论文要点.md)（同族 ViT/CSA ） |
...
| 仓库节点                                                                                                         | 关系                                                                                               |
| ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| [DeepSeek-V4](03-V4.md) / [CSA·HCA](05-CSA-HCA混合压缩注意力.md)                                             | V4.1 的 **直接前代**；CSA2 / CED / SWA Replay 是 V4 压缩注意力与异构 KV 路线的下一跳                                  |
| [Index Share](../05-DSA稀疏注意力/05-Index-Share梗概.md)                                                                              | 论文将跨层 Top-K 复用列为先验；CSA2 把 index 复用与 main KV 共享做成 **一等架构模式**                                      |
| [ESS](../06-推理基础设施/01-ESS概念.md) / [HiSparse](../06-推理基础设施/06-V4-HiSparse.md) / [磁盘 Prefix](../06-推理基础设施/07-V4-磁盘Prefix-Cache.md) | 同属「长上下文内存层级」；V4.1 用 **SWA Bounded Replay + 全局 KV 再压** 改写持久化策略                                    |
| [DSpark](../06-推理基础设施/04-DSpark投机解码.md)                                                                   | 官方组件；骨干无 MTP 预训练，DSpark **后训接入**                                                                 |
| [Engram](../07-Engram/01-Engram官方README.md)                                                                                | 从支线模块进入 **V4.1 骨干标配**（196B）                                                                      |
| [mHC](04-mHC流形约束超连接.md)                                                                   | Single-Pass mHC / Mega-mHC 内核融合                                                                  |
| [DeepSeek Harness](../10-Agent运行时/01-DeepSeek-Harness.md)                                                                          | HF README：代码 agent 榜（Terminal-Bench / DeepSWE 等）用 **DSH Minimal**（及 Standard / PTC）等 scaffold 评测 |
| [Visual Primitives](../09-附录/03-Visual-Primitives论文要点.md)                                 | 同族「V4-Flash + ViT」多模态叙事；V4.1 是 **从预训练起** 的原生多模态旗舰 Flash                                          |
...
- **Scaffold 敏感性**：同一模型在 Claude Code / Codex / DSH Minimal 等上分数不同 —— 读榜时要带上 **harness 名**（与 [ds-harness](../10-Agent运行时/01-DeepSeek-Harness.md) 文档互证）。
