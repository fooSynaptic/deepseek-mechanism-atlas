...
> [← 中文导读](../00-前言/02-中文导读.md) · [← 仓库首页（EN）](../../README.md) · [← V1 正文](00-V1-LLM.md) · [← 演进总览 §3.1 V1](../01-总览/01-版本演进总览.md#31-deepseek-llm-v1) · [V1→V3 演进](../01-总览/04-V1到V3演进.md)  
...
**编码**（推理时）：对输入做同样的预切分后，按 merge 优先级 **贪心最长匹配**（或等价 trie 查表），得到 token id 序列。解码为 merge 的逆过程：id → subword 字符串 → 拼接还原文本。
...
Tokenizer 训练语料来自 V1 **三阶段数据管线** 的多语言子集（与 [V1 §2.1 数据](00-V1-LLM.md#21-数据) / [#tokenizer](00-V1-LLM.md#tokenizer) 同一论文段落）：

```
原始语料 → 去重 → 过滤 → 重混 → BBPE 训练 / 模型预训练
```
...
| 实现 | 多种 | HuggingFace **tokenizers** 库（论文 §2.1） |
...
[直接打开 SVG](figures/v1/bbpe/bbpe-process-example.svg)
...
| **V3.2 / R1** | 与 V3 **同词表** |
| **V4** | 随新旗舰再扩展（见 [v4.md](03-V4.md) 训练节） |
...
3. 本地精读：[V1 §2.1 Tokenizer](00-V1-LLM.md#tokenizer) · [technical-highlights（复现向）](00-V1-BBPE词表与Tokenizer.md)
...
| ← 上一章 | 下一章 → |
|----------|----------|
| [DeepSeek-LLM V1](00-V1-LLM.md) | [DeepSeek-V2 梗概](00-V2-MoE与MLA.md) |
